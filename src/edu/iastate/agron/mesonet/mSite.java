 ...
 public mSite(String INportfolio, String idnum){
  ...
  try{
    ResultSet rs = dbInterface.callDB("SELECT * from iem_sites WHERE "
     +" s_mid = '"+idnum+"' and portfolio = '"+ this.portfolio+"' ");
    while ( rs.next() ){
 ...
 */ 
 public void update(){
   dbInterface.updateDB("UPDATE iem_sites SET s_name = '"+name+"', "
    +" s_hid = '"+ hid +"', s_nid = '"+ nid +"', s_mid = '"+mid+"', "
    +" s_lat = "+ lat +", s_lon = "+ lon +", s_st = '"+ state +"', "
