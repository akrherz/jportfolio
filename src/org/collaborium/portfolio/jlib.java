...
public class jlib {
  ...
  */
  public static String signUpForPortfolio(portfolioUser thisUser, HttpServletRequest req)
  {
  	...
  	String passwd2 = null;
  	try{
  		rs = dbInterface.callDB("SELECT passwd from portfolios WHERE portfolio = '"+portfolio+"' ");
  		rs.next();
  	...
  	try{
      updateDB("INSERT into students(username, portfolio) VALUES('"+thisUser.getUserID()+"', '"+portfolio+"')");  
      if (portfolio2 != null)
      { 
        updateDB("INSERT into students(username, portfolio) VALUES('"+thisUser.getUserID()+"', '"+portfolio2+"')");  
      }
  	} catch (Exception ex) {
  	}
  	
	sbuf.append( topBox("Sign In:") );
  	sbuf.append("Signed Up Successfully! Now you can return to the portfolio manager and log into your newly created"
  		+" portfolio");
	sbuf.append( botBox() );
  ...
  */ 
  public static String registerPortfolio( portfolioUser thisUser) {
	...
	sbuf.append("<P><B>Registration Password:</B> <BR> &nbsp;\n"
		+" <br />\n"
 ...
 */
  ...
  public static String basicHeader(portfolioUser thisUser, String title) {
	...
	if (thisUser == null) {
  	...
  	sbuf.append("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2//EN\">\n"
		...
		+ "<HEAD>\n"
		+ " \t<TITLE>" + title +  "</TITLE>\n"
		+ " \t<link rel=stylesheet type=text/css href="+httpBase+"/css/screen.css>\n"
	...
	sbuf.append("<div id=\"header-auth\"><B>User:</B></I> "+ realname +" ( "+ portfolio +" ) </div>\n");
  ...
  public static String genHeader(portfolioUser thisUser, String title, String selected) {
	...
	if (title == null)
  	...
  	sbuf.append("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2//EN\">\n"
		...
		+ "<head>\n"
		+ " \t<title>" + title +  "</title>\n"
		+ " \t<link rel=\"stylesheet\" type=\"text/css\" href=\""+httpBase+"/css/screen.css\" media=\"all\">\n"
		+ " \t<link rel=\"stylesheet\" type=\"text/css\" href=\""+httpBase+"/css/print.css\" media=\"print\">\n"
  +"<script src=\""+httpBase+"/js/portfolio.js\" type=\"text/javascript\"></script>\n"
  +"</head>\n"
		+ "<body>\n"
  ...
  */
  public static String getInstructor(String portfolio) {
  	try{
		ResultSet rs = dbInterface.callDB("SELECT admin from admins WHERE portfolio = '"+portfolio+"' ");
		rs.next();
  ...
  */
  public static boolean isMember(String username, String portfolio) {
  	try{
		ResultSet rs = dbInterface.callDB("SELECT * from students "
			+" WHERE username = '"+username+"' and portfolio = '"+portfolio+"' ");
  ...
  */
  public static String footer()
  {
	...
	theBuffer.append("<br clear=\"all\" /><div class=\"portfolio-footer\">Portfolio Application Suite" 
  ...
  */        
  public static String adminCommands()
  {
	...
	myBuffer.append("<ul><li><a "
		+" href='"+servletHttpBase+"/jportfolioAdmin'>Portfolio Manager</a></li>\n"
		+" <li><a "
  ...
  */
  public static java.sql.Timestamp lastLogin(String username){
  	...
  	try{
		ResultSet rs = dbInterface.callDB("SELECT login from sessions WHERE username "
...
//	myBuffer.append( allUsers.toString() + "Hello");
	...
	if (!allUsers.isEmpty() ) {
		do {
			...
			//plogger.report("ThisUser is :"+thisUser +": NowUser is :"+nowUser+":\n"); 
			...
			if (nowUser.equalsIgnoreCase(thisUser) ){
				...
				myBuffer.append("<a href=\"javascript:new_window('/jportfolio/jsp/user/IMPost.jsp?toUser="+tmp.toString()+"');\">"
					+  nowUser +"</a>: ");
			}
	...
	if ( IMDatabase.hasMessage(thisUser).booleanValue() )
		myBuffer.append("<BR><a href=\"javascript:new_window('/jportfolio/jsp/user/IMCat.jsp');\">You have an IM</a>");
...
//	myBuffer.append("   <a class=\"commands\" href='"+servletHttpBase+"/jportfolio'>Portfolio Manager<a/><br>\n"
//		+"   <a class=\"commands\" href='"+httpBase+"/jsp/user/myCalendar.jsp'>Calendar</a><BR>\n"
//		+"   <a class=\"commands\" href='"+servletHttpBase+"/ChatDispatch'>Chat</a><BR>\n"
//		+"   <a class=\"commands\" href='"+servletHttpBase+"/jdot3'>Dialog</a><br>\n"
//		+"   <a class=\"commands\" href='"+servletHttpBase+"/forecast'>Forecast</a><br>\n"
//		+"   <a class=\"commands\" href='"+servletHttpBase+"/jquiz'>Quiz<a/><br>\n"
//		+"   <a class=\"commands\" href='"+servletHttpBase+"/jportfolio?mode=l'>Log Out</a>\n");
...
// 	myBuffer.append( botBox() );
  ...
  */
  public static String getName(String userID) {
	...
	try {
		//rs = callDB("select fname, lname from users where username = '"+userID+"' ");
  ...
  */
  ...
  public static String basicSideBar() {
	...
	sbuf.append("If you have any questions about this system, please send an email to \n"
		+"<a href=\"mailto:systems@iitap.iastate.edu\">systems@iitap.iastate.edu</a>\n");
  ...
  public static boolean isAdmin(String user, String portfolio) {
	...
	try{
		ResultSet rs = dbInterface.callDB("SELECT admin as admin1 from admins "
			+" WHERE portfolio = '"+portfolio+"' and admin = '"+user+"' ");
		if ( rs.next() )
			return true;
		else	
			return false;
  ...
  */
  ...
  public static String groupMemberListing(portfolioUser thisUser){
   ...
   sbuf.append("<TABLE>\n");
  	try{
	ResultSet rs = dbInterface.callDB("SELECT getUserName(username) as realname, "
		+" getEmail(username) as email, username from students "
		+" WHERE portfolio = '"+ thisUser.getPortfolio() +"' and "
		+" gid = '"+ thisUser.getGroupID() +"' ");
	while (rs.next() ) {
	  sbuf.append("<TR><TD><a href=\"/jportfolio/users/"+ rs.getString("username")+"\"> "
  ...
  */
  public static String cleanGetString(String source)
  {
	...
	for (int i = 0; i < source.length(); i++) 
		...
		if (temp.equals(" ") ) { myBuffer.append("%20"); }
  ...
  */
  public static String printAllMessages( portfolioUser thisUser ) 
  {
    ...
    try{
      ...
      rs = dbInterface.callDB("select to_char(entered, 'Mon dd') as d, "
      ...
      while (rs.next() ) {
        ...
        sbuf.append("<TR>\n"
         ...
         +"<TD>"+message+" :</TD>\n"
         +"<TD><a href=\""+program+"\">View</a>\n &nbsp; <b>/</b> &nbsp;"
         +"<a href=\""+notifyBaseURL+"?mode=o&idnum="+idnum+"\">Clear</a></TD>\n"
         +"</TR>\n");
      }
    ...
    try{
     ...
     if (rs.next())
     {
      ...
      while (rs.next() ) {
       ...
       if ( rs.previous() )
         sbuf.append("<TR><TD colspan=\"5\"><a href=\"/jportfolio/jsp/user/myNotify.jsp\">View All</a></TD></TR>\n");
		else{
  ...
  */
  public static String topicsSelect(portfolioUser thisUser, String blockid){
    ...
    try{
      ResultSet rs = dbInterface.callDB("select name, blockid, unitid, "
	+" blockid || '.' || unitid || ' ' || title as uname from units "
	+" WHERE portfolio = '"+ thisUser.getPortfolio() +"' ORDER by blockid,unitid ASC");
  ...
  */
  public static String getMotd(String portfolio) {
  	StringBuffer sbuf = new StringBuffer();
	try{
		ResultSet rs = dbInterface.callDB("SELECT id, body, to_char(issue, 'DD Month YYYY HH12:MI AM') as issued, issue "
			+" from motd WHERE portfolio = '"+portfolio+"'  ORDER by issue DESC LIMIT 1");
		rs.next();
		sbuf.append("<P>Posted on: "+ rs.getString("issued") +"<BR />\n");
        String body = rs.getString("body");
        if (body.length() > 1000){ 
          sbuf.append( body.substring(0,990)
           +"<p><i>Message truncated...</i><a "
           +" href=\"/jportfolio/jsp/user/printMotd.jsp?idnum="+ rs.getString("id") +"\">View Entire MOTD</a><br />");
        } else {
          sbuf.append( body );
       }
  ...
  public static String topBoxNoBR(String title) {
 	...
 	sbuf.append("\n\n\t<!-- Begin box element for "+title+" -->\n");
  	sbuf.append("<table width='100%' border='0' rowspacing='0' cellpadding='3' cellspacing='0'>\n");
	sbuf.append("<tr><td class=\"topBox\" NOWRAP>"
	//	+ "<font size=+1 color=\""+boxTitleTextColor+"\"><i>"+title+"</i></font>"
		+ title + "</td></tr>\n");
  ...
  */
  public static Boolean userIDExists(String userID) {
	...
	try{
		ResultSet rs = dbInterface.callDB("SELECT username from users WHERE username = '"+userID+"' ");
		if ( rs.next() )
  			return Boolean.TRUE;
		else
