...
public class portfolioMessage {
  ...
  */
  ...
  private void getMessage() throws SQLException{
  	ResultSet rs = dbInterface.callDB("SELECT * from dialog "
...
//    plogger.report(authorName +"::"+ subject +"::"+ body );
    dbInterface.updateDB("insert into dialog (username, name, "
     ...
     +" '"+security+"', "+gid+" , '"+stringUtils.cleanString(link)+"', "
     +" '"+topicid+"')");
  }
  ...
  */
  public String printStandard(){
    ...
    sbuf.append("<div class=\"dialog-post-header\">\n"
	  + newStr + "<h4 class=\""+ classification +"\">"+ subject +" ("+classification+")</h4>\n"
      +" Posted by <a href=\"/jportfolio/users/"+ author +"\">"+ authorName +"</a>");
    if (this.role != null) { sbuf.append(" ("+ this.role +") "); }
...
//	if (thisUser != null ) {
//	
//	  if (thisUser.getUserID().equalsIgnoreCase(author) || thisUser.isAdmin() ){
//	    
...
//		+ tempCat_smiles +"<a href=\"javascript:new_window('/jportfolio/jsp/jdot/cat_smile.jsp?idnum="+ idnum +"');\">"
//		+"<img src=\"/jportfolio/images/icon_smile.gif\" border=\"0\"></a>\n"
//		+ tempCat_frowns +"<a href=\"javascript:new_window('/jportfolio/jsp/jdot/cat_frown.jsp?idnum="+ idnum +"');\">"
//		+"<img src=\"/jportfolio/images/icon_frown.gif\" border=\"0\"></a>\n");		

//	sbuf.append("<BR><b>This post contributed to my learning:</b>\n"
//                + tempLearn_smiles +"<a href=\"javascript:new_window('/jportfolio/jsp/jdot/learn_smile.jsp?idnum="+ idnum +"');\">"
//                +"<img src=\"/jportfolio/images/icon_smile.gif\" border=\"0\"></a>\n"
//                + tempLearn_frowns +"<a href=\"javascript:new_window('/jportfolio/jsp/jdot/learn_frown.jsp?idnum="+ idnum +"');\">"
//                +"<img src=\"/jportfolio/images/icon_frown.gif\" border=\"0\"></a><BR>\n");
		
		
	// Here is where we set a limit on how many sub-posts there can be, currently we only
	// allow a 6 deep thread, so if the idnum is 6x5 chars in length, we had better not allow
	// them to reply to this post
	if ( idnum.length() < 30)
		sbuf.append("\n<a href='"+thisPageURL+"?mode=p&threadid="+ threadID +"&idnum="+ idnum +"'>"
			+"Post a follow up</a>\n");
	else
	...
	if ( ! numMessages.equalsIgnoreCase("0")  ) {
		...
		sbuf.append("\n(Follow-Ups: " + numMessages +") "
			...
			+" &nbsp; | &nbsp; <a href='"+thisPageURL+"?mode=e&threadid="+ threadID +"'>Overview</a>"
			+ "<br> ");
	}
  	sbuf.append("</div>\n");
  ...
  public String subPosts() {
    ...
    try {
      ResultSet rs2 = dbInterface.callDB("select count(name) as result "
  ...
  */
  public String printLong(String thisPageURL) {
  	...
  	try{
		sbuf.append("<P><i>subject:</i><font size=+1 "
			...
			+ subject +" ("+ classification +")</b></font>\n");
	} catch(Exception ex) {
		sbuf.append("<P><i>subject:</i><font size=+1><b>"
			+ subject +" ("+ classification +")</b></font>\n");
	}
	
	// More info
    sbuf.append("\n<BR><i>author:</i>"
      +"<a class='commands' href='/jportfolio/users/"+ author +"'>" + authorName + "</a> "
      +"\n<BR><i>date:</i>" + stringUtils.gmtDate( getDate() ) 
      +"\n<BR><i>posts:</i> Currently "+numMessages+" responses\n");
	sbuf.append("\n<BR><i>groupID:</i>" + gid );
	...
	if (idnum.length() < 30 )
	sbuf.append("\n<BR><BR>| <a href='"+thisPageURL+"?mode=p&threadid="+threadID+"&idnum="+idnum+"'>"
		+"Post a follow up</a> |\n");
	return sbuf.toString();
  }
  ...
  public String printShort(String thisPageURL) {
  	...
  	sbuf.append("<LI>");
	try{
		sbuf.append("<font size=+1 class=\""+security+"\"><b>"
			+ subject +" ("+security+")</b></font>\n");
	} catch(Exception ex) {
	...
	if (idnum.length() < 30 )
	sbuf.append("\n<BR>| <a href='"+thisPageURL+"?mode=p&threadid="+threadID+"&idnum="+idnum+"'>"
		+"Post a follow up</a>\n");
...
public void addCatFrown(){
 ...
 public void addLearnFrown(){
        try{
                dbInterface.updateDB("UPDATE dialog SET learn_frown = learn_frown + 1 WHERE "
                +" idnum = "+ idnum +" ");
...
public void addLearnSmile(){
        try{
                dbInterface.updateDB("UPDATE dialog SET learn_smile = learn_smile + 1 WHERE "
                +" idnum = "+ idnum +" ");
