...
private String ClocalSnowNum = "-99";
  ...
  public forecastDay(String newPortfolio, String newSqlDate){
	...
	this.sqlDate = newSqlDate;
  	...
  	try{
		ResultSet rs = dbInterface.callDB("SELECT * from forecast_days "
 ...
 public boolean getValidation() {
	...
	try{
		...
		ResultSet rs = dbInterface.callDB("SELECT * from forecast_answers "
		...
		if (rs.next() ) {
			...
			//plogger.report("I am setting VlocalSnowCat to "+ rs.getString("local_snow") );
 ...
 public boolean getClimo() {
	...
	try{
		...
		ResultSet rs = dbInterface.callDB("SELECT * from forecast_climo "
			+" WHERE portfolio = '"+ portfolio +"' "
			+" and day = '"+ sqlDate +"' ");
		if (rs.next() ) {
  ...
  */
  public String catAnswers() {
    ...
    sbuf.append("<P><TABLE border=1>\n"
      +"<tr><th colspan=3>Forecast Verification for: "+ sqlDate+" </th></tr> \n"
      +"<TR><TD></TD><TD>"+ localSite +"( "+ localSiteID +" ) </TD>\n"
      +" <TD>"+ floaterSite +"( "+ floaterSiteID +" ) </TD></TR>\n"
  
      +" <TR><TH>High Temp:</TH><TD>"+ VlocalHighTemp.toString() +"</TD>\n"
  ...
  */
  public boolean getForecast() {
    ...
    try{
      ResultSet rs = dbInterface.callDB("SELECT * from forecasts "
