...
public class forecast extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // n == select a certain day to view a users forecast for
        {
            ...
            case 'l':
                if (sqlDate != null)
                    pageContent.append("<P align=\"right\">"
                            + "<a href=\"/jportfolio/jsp/user/forecast/prevForecast.jsp?sqlDate=" + sqlDate
                            + "\">Printable Version</a>\n");
                else
                    pageContent.append("<P align=\"right\">"
            ...
            while (rs.next()) {
                ...
                if (thisUser.isAdmin()) {
                    sbuf.append("<tr><td></td><td><i>Coinfidence:</i>  "
                            ...
                            + "<tr><td></td><td colspan=8>" + stringUtils.toBR(rs.getString("discussion"))
                            + "</td></tr>\n");
                }
            }
        } catch (Exception ex) {
    ...
    public String makeForecast(portfolioUser thisUser, String sqlDate,
        ...
        sbuf.append("<form method='POST' action='" + thisPageURL + "'>\n"
                ...
                + " <TD><input type='text' size='4' MAXLENGTH='3' name='local_low' value='" + thisDay.getLocalLowTemp()
                + "'></TD>\n"
                ...
                + " <input type=\"text\" name=\"confidence\" value=\"" + thisDay.getConfidence()
                + "\" size=2 maxsize=2>"

                + "<p><b>Discuss your thoughts regarding this forecast.</b>"
                + "<br>You will <b>not</b> be graded for this discussion."
                + "<br><textarea name=\"discussion\" cols=50 rows=4>" + thisDay.getDiscussion()
                + "</textarea></td></tr>\n"
                + "</TABLE>\n");
