...
public class forecast extends HttpServlet {
	...
	// n == select a certain day to view a users forecast for
	{
	 case 'c':
	 	pageContent.append("<P align=\"right\">"
	  	...
	  	try {
		  	pageContent.append( fLib.cumulativeResults(thisUser.getPortfolio(), 
				sort, thisPageURL) );
		} catch ( Exception ex) {
	  ...
	  case 'l':
	  	if (sqlDate != null) 
		  	pageContent.append("<P align=\"right\">"
			+"<a href=\"/jportfolio/jsp/user/forecast/prevForecast.jsp?sqlDate="+sqlDate+"\">Printable Version</a>\n");
		else
			pageContent.append("<P align=\"right\">"
	  	...
	  	try {
		  	pageContent.append( fLib.forecastResults(thisUser.getPortfolio() , sqlDate, 
				sort, thisPageURL) );
		} catch ( Exception ex) {
	  ...
	  case 'm':
	  	try {
		  	pageContent.append( fLib.whichDay( thisUser.getPortfolio() , "l", thisPageURL) );
		} catch (Exception ex) {
	  ...
	  case 'n':
	  	try{
			pageContent.append( fLib.whichDay( thisUser.getPortfolio(), "o", thisPageURL) );
		} catch(Exception ex){
  ...
  */
  public String userForecastResults(portfolioUser thisUser, String sqlDate) {
    ...
    sbuf.append("<br><b>Confidence:</b> "+ thisDay.getConfidence() 
      +"<br><b>Discussion:</b> "+ stringUtils.toBR(thisDay.getDiscussion()) );
	...
	sbuf.append("<P><TABLE>\n"
		...
		+"<TR><TH rowspan=2>Forecaster</TH><TD colspan=4>"+ thisDay.getLocalSite() +"</TD>\n"
    ...
    try{
      ResultSet rs = dbInterface.callDB("SELECT getUserName(userid) as rname, "
      ...
      while (rs.next() ){
        ...
        if (thisUser.isAdmin()){
          sbuf.append("<tr><td></td><td><i>Coinfidence:</i>  "
            + rs.getString("confidence") +"</td><td colspan=7></td></tr>\n"
            +"<tr><td></td><td colspan=8>"+ stringUtils.toBR(rs.getString("discussion")) +"</td></tr>\n");
        }
      }
    } catch(Exception ex){
  ...
  */
  public String mkIntro() {
	...
	sbuf.append("<P>This is the Forecasting Activity Application inside of Portfolio.  Your \n"
		+" portfolio may not be participating in the Activity, so this page may be ignored. \n");

	sbuf.append("<P>In the column to the left, you will find options available for this portfolio \n"
  ...
  public String classOptions( portfolioUser thisUser, String thisPageURL ) {
	...
	if (isForecastDay( thisUser )) 
		sbuf.append("<LI><a href='"+thisPageURL+"?mode=f'>Make Forecast</a></LI>\n");
	else
		sbuf.append("No Forecast for Today!");

	sbuf.append("<LI><a class=\"commands\" href='"+thisPageURL+"?mode=m'>Previous Forecast Results</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=l'>Last Forecast Results</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=c'>Cumulative Standings</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=n'>View Forecasts</a></li>\n");
    sbuf.append("</ul>\n");
  ...
  */
  public String makeForecast(portfolioUser thisUser, String sqlDate, 
    ...
    if ( thisDay.getForecast() )
      ...
      sbuf.append("<P>This is your first attempt at this forecast. Good luck.\n");
    ...
    sbuf.append("<form method='POST' action='"+thisPageURL+"'>\n"
      ...
      +" <TR><TH>Local:</TH>\n"
      +" <TD><input type='text' size='4' MAXLENGTH='3' name='local_high' value='"+ thisDay.getLocalHighTemp() +"'></TD>\n"
      +" <TD><input type='text' size='4' MAXLENGTH='3' name='local_low' value='"+ thisDay.getLocalLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.rainSelect("local_prec", thisDay.getLocalPrecCat() )+"</TD>\n"
		+"	<TD>"+fLib.snowSelect("local_snow", thisDay.getLocalSnowCat() )+"</TD>\n"
		+"	</TR>\n"		
			
		+" <TR><TH>"+ thisDay.getFloaterSite() +":</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_high' value='"+ thisDay.getFloaterHighTemp() +"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_low' value='"+ thisDay.getFloaterLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.rainSelect("float_prec", thisDay.getFloaterPrecCat() )+"</TD>\n"
		+"	<TD>"+fLib.snowSelect("float_snow", thisDay.getFloaterSnowCat() )+"</TD>\n"
		+"	</TR>"
    ...
    +" <input type=\"text\" name=\"confidence\" value=\""+ thisDay.getConfidence() +"\" size=2 maxsize=2>"

   +"<p><b>Discuss your thoughts regarding this forecast.</b>"
   +"<br>You will <b>not</b> be graded for this discussion."
   +"<br><textarea name=\"discussion\" cols=50 rows=4>"+ thisDay.getDiscussion() +"</textarea></td></tr>\n"
    +"</TABLE>\n");
  ...
  */
  public boolean isForecastDay( portfolioUser thisUser) {
	...
	try {
		ResultSet rs = dbInterface.callDB("SELECT day from forecast_days "
			+" WHERE portfolio = '"+ thisUser.getPortfolio() +"' "
			+" and day = 'TODAY'::date + '1 day'::interval ");
	  	if ( rs.next() ) {
			ResultSet rs2 = dbInterface.callDB("SELECT * from portfolios "
  ...
  if (confidence.equals("")) confidence = "0";
  ...
  if (Int_local_high.intValue() < Int_local_low.intValue() ) {
	...
	try {
		jlib.updateDB("DELETE from forecasts WHERE userID = '"+ thisUser.getUserID()+"' "
  ...
  public String selectForecastDays(portfolioUser thisUser, String selected) {
  	...
  	try{
  		...
  		while (rs.next() ){
  			...
  			if (selected.equalsIgnoreCase(thisVal) ) sbuf.append("SELECTED");
  			sbuf.append(">"+thisVal+"\n");
  		}
  	} catch(Exception ex) {
