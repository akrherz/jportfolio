...
public class forecastAdmin extends HttpServlet {
  ...
  */
  	...
  	throws IOException, ServletException
  {
	...
	// n == I have stuid and sqlDate, lets modify forecast
	  ...
	  break;
	  case 'n':
	  	pageContent.append( makeForecast( thisUser, selectedUserID, sqlDate, thisPageURL ) );
	  break;
	  case 'a':
	  ...
	  case 'u':
		try{
		  pageContent.append( fLib.whichDay( thisUser.getPortfolio() , "v", thisPageURL ));
		} catch( Exception ex) {}
	  break;
	  case 'v':
	  ...
	  case 'w':
	  	try{
		  pageContent.append( fLib.whichDay( thisUser.getPortfolio() , "x", thisPageURL ));
		} catch( Exception ex) {}
	  break;
	  case 'x':
  ...
  */
	 	...
	 	case 'f':
			...
			try {
				...
				pageContent.append("<P>An error occured processing your forecast"
					+"<BR> "+ ex +". <BR>Please try to forecast Again!");
			}
			
			pageContent.append("<P>Re-running Grading for this day...<BR>\n");
			try{
				pageContent.append( fLib.gradeForecasts( thisUser.getPortfolio(), sqlDate ) );
  ...
  public String viewAllForecasts( portfolioUser thisUser , String sqlDate )
  {
	...
	sbuf.append("<HR><TABLE><TR>\n"
		+"<TH rowspan=\"2\">Forecaster:</TH><TH colspan=\"4\">"+ thisDay.getLocalSite()
		+"</TH><TH colspan=\"4\">"+ thisDay.getFloaterSite() +"</TH></TR>\n"
		+"<TR>\n"
		+"<TD>High:</TD><TD>Low:</TD><TD>Prec Cat:</TD><TD>Snow Cat:</TD>\n"
		+"<TD>High:</TD><TD>Low:</TD><TD>Prec Cat:</TD><TD>Snow Cat:</TD>\n"
		+"</TR>\n");
	try{
		ResultSet rs = dbInterface.callDB("SELECT *, getUserName(userid) as rname from forecasts WHERE"
		+" portfolio = '"+ thisUser.getPortfolio() +"' and day = '"+ sqlDate +"' ");
		while ( rs.next() ){
  ...
  public String whatTime2Forecast( portfolioUser thisUser, String thisPageURL )
  {
	...
	sbuf.append("<blockquote class=\"instructions\">You must specify a local time at which the\n"
	...
	try {
		ResultSet rs = dbInterface.callDB("SELECT fxtime from portfolios WHERE "
			+" portfolio = '"+ thisUser.getPortfolio() +"' and fxtime IS NOT NULL");
		if ( rs.next() ) 
			sbuf.append("<input type=\"text\" name=\"fxtime\" value "
				+" =\""+ rs.getString("fxtime") +"\" size=\"2\" maxlength=\"2\">\n");
		else
  ...
  public String makeForecast(portfolioUser thisUser, 
  		String selectedUserID, String sqlDate, String thisPageURL)
 {
	StringBuffer sbuf = new StringBuffer();
	...
	try{
		ResultSet rs = jlib.callDB("SELECT * from forecasts "
			+" WHERE day = '"+sqlDate+"' and userID = '"+ selectedUserID +"' "
	...
	sbuf.append("<form method='POST' action='"+thisPageURL+"'>\n"
		...
		+" <TR><TH>Local:</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='local_high' value='"+local_high+"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='local_low' value='"+local_low+"'></TD>\n"
		+"	<TD>"+fLib.rainSelect("local_prec", local_prec)+"</TD>\n"
		+"	<TD>"+fLib.snowSelect("local_snow", local_snow)+"</TD>\n"
		+"	</TR>\n"		
			
		+" <TR><TH>Floater:</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_high' value='"+float_high+"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_low' value='"+float_low+"'></TD>\n"
  ...
  public String enterForecast(portfolioUser thisUser, String selectedUserID, HttpServletRequest req)
	throws myException {
 	StringBuffer sbuf = new StringBuffer();
	...
	String colNames = "(userID, portfolio, day, local_high, local_low, local_prec, local_snow, "
		+" float_high, float_low, float_prec, float_snow )";
	String colVals = "('"+ selectedUserID +"', '"+thisUser.getPortfolio()+"', '"+ sqlDate +"', "
		+ local_high+", "+local_low+", "+local_prec+", "+local_snow+", "
		+ float_high+", "+float_low+", "+float_prec+", "+float_snow+" )";
	
	try {
		jlib.updateDB("DELETE from forecasts WHERE userID = '"+  selectedUserID +"' "
			+" and portfolio = '"+ thisUser.getPortfolio() +"' and day = '"+sqlDate+"' ");
  ...
  */
  public String alterStudent(portfolioUser thisUser, String thisPageURL) {
  	...
  	sbuf.append("<P>Please select which user's forecast you want to meddle with.\n");
	...
	try{
 		ResultSet rs = dbInterface.callDB("SELECT getUserName(s.username) as name, s.username "
			+" from students s, users u"
			+" WHERE s.portfolio = '"+ thisUser.getPortfolio() +"' and s.username = u.username "
			+" and s.nofx = 'n' ");
 		while (rs.next() ) {
 			sbuf.append("<OPTION VALUE=\""+rs.getString("username")+"\">"+rs.getString("name")+" \n");	
 		}
 	} catch(Exception ex) {
	...
	try{
		ResultSet forecastDays = dbInterface.callDB("SELECT * from forecast_days WHERE "
  ...
  */
  public String whoForecasts(portfolioUser thisUser , String thisPageURL) {
  	...
  	sbuf.append("<blockquote><font class=\"instructions\">Listed below are \n"
  	...
  	try {
  	ResultSet rs = dbInterface.callDB("SELECT getUsername(username) as uname, * from "
	...
	while (rs.next() ) {
		...
		if ( nofx != null && nofx.equalsIgnoreCase("t") )
			...
			+ "&changeuser="+ thisUserID+"&changeto=n\">"+ thisUserName+"</a> "
			+ " does not forecast.\n");
		else
			sbuf.append("<P><a href=\""+thisPageURL +"?mode=b"
			+ "&changeuser="+ thisUserID+"&changeto=y\">"+ thisUserName+"</a> "
			+ " does forecast.\n");
  ...
  */
  public String createForecastDay(String thisPageURL) {
	...
	sbuf.append("Select the day that the forecast will be made for, <B>NOT</B> the \n"
	...
	sbuf.append("<H3>Input the station code:</H3>\n"
  ...
  */
  public String createValidation(portfolioUser thisUser, String sqlDate) {
	...
	sbuf.append("<form method='POST' action='"+thisPageURL+"'>\n"
		...
		+" <TR><TH>Local:</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='local_high' value='"+ thisDay.getVLocalHighTemp() +"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='local_low' value='"+ thisDay.getVLocalLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.adminRainSelect("local_prec", thisDay.getVLocalPrecCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"local_prec_txt\" value=\""+ thisDay.getVLocalPrecNum() +"\"></TD>\n"
		+"	<TD>"+fLib.adminSnowSelect("local_snow", thisDay.getVLocalSnowCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"local_snow_txt\" value=\""+ thisDay.getVLocalSnowNum() +"\"></TD>\n"
		+"	</TR>\n"		
			
		+" <TR><TH>"+ thisDay.getFloaterSiteID() +":</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_high' value='"+ thisDay.getVFloaterHighTemp() +"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='float_low' value='"+ thisDay.getVFloaterLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.adminRainSelect("float_prec", thisDay.getVFloaterPrecCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"float_prec_txt\" value=\""+ thisDay.getVFloaterPrecNum() +"\"></TD>\n"
		+"	<TD>"+fLib.adminSnowSelect("float_snow", thisDay.getVFloaterSnowCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"float_snow_txt\" value=\""+ thisDay.getVFloaterSnowNum() +"\"></TD>\n"
		+"	</TR>\n"
		
		+" <TR><TH>Climo Local:</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='cl_local_high' value='"+ thisDay.getCLocalHighTemp() +"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='cl_local_low' value='"+ thisDay.getCLocalLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.rainSelect("cl_local_prec", thisDay.getCLocalPrecCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"cl_local_prec_txt\" value=\""+ thisDay.getCLocalPrecNum() +"\"></TD>\n"
		+"	<TD>"+fLib.snowSelect("cl_local_snow", thisDay.getCLocalSnowCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"cl_local_snow_txt\" value=\""+ thisDay.getCLocalSnowNum() +"\"></TD>\n"
		+"	</TR>\n"
		
		+" <TR><TH>Climo "+ thisDay.getFloaterSiteID() +":</TH>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='cl_float_high' value='"+ thisDay.getCFloaterHighTemp() +"'></TD>\n"
		+"	<TD><input type='text' size='4' MAXLENGTH='3' name='cl_float_low' value='"+ thisDay.getCFloaterLowTemp() +"'></TD>\n"
		+"	<TD>"+fLib.rainSelect("cl_float_prec", thisDay.getCFloaterPrecCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"cl_float_prec_txt\" value=\""+ thisDay.getCFloaterPrecNum() +"\"></TD>\n"
		+"	<TD>"+fLib.snowSelect("cl_float_snow", thisDay.getCFloaterSnowCat() )+"</TD>\n"
		+"	<TD><input type=\"text\" size=\"10\" name=\"cl_float_snow_txt\" value=\""+ thisDay.getCFloaterSnowNum() +"\"></TD>\n"
		+"	</TR>\n"
  ...
  public String enterForecastValidation(portfolioUser thisUser, HttpServletRequest req) {
 	StringBuffer sbuf = new StringBuffer();
	...
	String colNames = "(portfolio, local_high, local_low, local_prec, "
	...
	String colVals = "('"+thisUser.getPortfolio()+"' "
		+", "+local_high+", "+local_low+", "+local_prec+", "+local_snow+", "+float_high+", "
		+" "+float_low+", "+float_prec+", "+float_snow+", '"+sqlDate+"', "
		+" '"+local_prec_txt+"', '"+ local_snow_txt +"', '"+ float_prec_txt +"', '"+ float_snow_txt+"')";
	
	String colNames2 = "(portfolio, local_high, local_low, local_prec, "
		+" local_snow, float_high, float_low, float_prec, float_snow, day, "
		+" local_prec_txt, local_snow_txt, float_prec_txt, float_snow_txt)";
	String colVals2 = "('"+thisUser.getPortfolio()+"' "
		+", "+cl_local_high+", "+cl_local_low+", "+cl_local_prec+", "+cl_local_snow+", "+cl_float_high+", "
		+" "+cl_float_low+", "+cl_float_prec+", "+cl_float_snow+", '"+sqlDate+"', "
		+" '"+cl_local_prec_txt+"', '"+ cl_local_snow_txt +"', '"+ cl_float_prec_txt +"', '"+ cl_float_snow_txt+"')";
	
	
	try {
		dbInterface.updateDB("DELETE from forecast_answers WHERE "
			+" portfolio = '"+ thisUser.getPortfolio() +"' and day = '"+sqlDate+"' ");
		dbInterface.updateDB("INSERT into forecast_answers "+colNames+" VALUES "+colVals+" ");
		
		dbInterface.updateDB("DELETE from forecast_climo WHERE  "
			+" portfolio = '"+ thisUser.getPortfolio() +"' and day = '"+sqlDate+"' ");
		dbInterface.updateDB("INSERT into forecast_climo "+colNames2+" VALUES "+colVals2+" ");
  ...
  */
  public String addForecastingDay(HttpServletRequest request, String portfolio) {
	StringBuffer sbuf = new StringBuffer();
	...
	try {
		...
		dbInterface.updateDB("INSERT into forecast_days VALUES('"+ portfolio +"', "
			+" '"+dateStr+"','"+station+"','"+code+"', "+ case_group +") ");
			
		dbInterface.updateDB("INSERT into calendar(portfolio, description, URL, valid) VALUES "
   ...
   public String adminOptions(portfolioUser thisUser, String thisPageURL) {
	...
	sbuf.append("<ul><li><a class=\"commands\" href='"+thisPageURL+"?mode=y'>Create a Forecast Day</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=u'>Enter Validation</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=a'>Admin Forecast Options</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=m'>Alter User's Forecast</a></li>\n");
	sbuf.append("<li><a class=\"commands\" href='"+thisPageURL+"?mode=w'>View All Forecasts</a></li></ul>\n");
	sbuf.append( jlib.botBox() );
