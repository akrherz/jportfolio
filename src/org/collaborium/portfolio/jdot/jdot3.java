  ...
  if (postedDialogType != null ) {
	...
	case 'o':
	   pageContent.append( jdot.readMore(thisUser, STRidnum, "0", 
	   		thisUser.getDialogSecurity(), thisPageURL) );
	break;
//	case 'b':
...
//	break;
	...
	break;
	case 'p':
		pageContent.append( jdot.postMessage(threadID, STRidnum, 
				thisPageURL, thisUser.getDialogSecurity() ) );
	break;  
	case 'n':
  ...
  */
  ...
  portfolioUser thisUser = (portfolioUser)session.getAttribute("User");
  portfolioMessage sMessage = (portfolioMessage)session.getAttribute("sMessage");
  ...
  switch ( callMethod.charAt(0) ) 
  {
    case 's':
      if (searchStr != null && ! searchStr.equals("") ) {
        ...
        try{
          pageContent.append( jdot.execSearch(thisUser, searchStr, 
                            searchCol, skipNum, thisPageURL) );
        } catch(Exception ex){
 ...
 /**
  * Method that prints out a listing of side bar items, should be modified to fit a certain application
  *
  */
