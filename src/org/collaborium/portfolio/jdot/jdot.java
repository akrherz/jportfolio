...
public class jdot{
  ...
  */
  public static String newThread(portfolioUser thisUser, String thisPageURL) {
    ...
    theBuffer.append("<FORM METHOD='POST' ACTION='"+thisPageURL+"'>\n "	 
      + "<P><B>Subject:</B><BR>\n"
      + "	<INPUT size=60 NAME='subject' Value=''>\n"
      + "<P><B>Classify Your Post:</B> <i>(Optional)</i> \n"
      +" <a href=\"/jportfolio/info/dialog.html\" target=\"_new\">More Information</a> about classifications.<BR>\n"
      + " <SELECT name='type'>\n"
      ...
      +"<P><B>Message:</B><br> "
      +" <TEXTAREA NAME='body' WRAP='Virtual' ROWS=\"20\" COLS=\"60\"></TEXTAREA>  "
      +"<BR>If you would like to include a link to another webpage, you can enter it "
      +" here:<BR>\n <INPUT TYPE='text' name='link' size='60'>\n"
      
      +"<p>(<i>Optional</i>)  If this post is a topic starter, please select the \n"
  ...
  */
  ...
  sbuf.append("<table width=\"100%\" border=0>\n"
    +" <tr><td><font class=\"bodyText\"><b>Restrict Thread Types:</b></font>\n"
    +" </td><td>\n"
    +" <form method=\"GET\" action=\""+ thisPageURL +"\">\n"
    +" <select name=\"threadType\" "
    +"   onChange=\"location=this.form.threadType.options[this.form.threadType.selectedIndex].value\">\n"
    +"   <option value=\""+thisPageURL+"?threadType=all\" ");
  if (threadType.equalsIgnoreCase("all") ) sbuf.append(" SELECTED ");
    sbuf.append(">ALL \n"	
      +"   <option value=\""+thisPageURL+"?threadType=ethical\" ");
  if (threadType.equalsIgnoreCase("ethical") ) sbuf.append(" SELECTED ");
    sbuf.append(">Ethical Discussion \n"	
      +"   <option value=\""+thisPageURL+"?threadType=unit\" ");
  if (threadType.equalsIgnoreCase("unit") ) sbuf.append(" SELECTED ");
  ...
  int skipMessenges = Integer.parseInt( skipNum) * messageBlock;
	...
	+" and idnum < 200000 and portfolio = '"+ thisUser.getPortfolio()
	+"' and security = 'group' "+ s1 ;
      else
	mySQL = "select count(name) as result from dialog where idnum > 10000 "
	+" and idnum < 200000 and portfolio = '"+thisUser.getPortfolio()
	+"' and security = 'group' "
	...
	mySQL = "select count(name) as result from dialog where idnum > 10000 "
	+" and idnum < 200000 and portfolio = '"+thisUser.getPortfolio()
	+"' and security = '"+thisUser.getDialogSecurity()+"' "+ s1;
    ...
    if (numMessages2 > messageBlock ) {
      ...
      for (int i = 0; i < ( (numMessages2-1) / messageBlock +1) ; i++ ) {
        if (!skipNum.equalsIgnoreCase( String.valueOf(i) ) ){
          sbuf.append("<a href='"+thisPageURL+"?skipNum="+i+"'>"+(i+1)+"</a> &nbsp;\n");
        } else {
          sbuf.append("<b>"+(i+1)+"</b> &nbsp;\n");
	}
      }
  ...
  if ( thisUser.getDialogSecurity().equalsIgnoreCase("group") )
     if ( thisUser.isAdmin() )
       mySQL2 = "select *, getRoleName(username, portfolio) as role from dialog where idnum > 10000 "
        +" and idnum < 200000 and portfolio = '"+thisUser.getPortfolio()
        +"' and security = 'group' "+ s1
        +" ORDER BY date DESC LIMIT "+messageBlock+" OFFSET "+skipMessenges ;
     else
       mySQL2 = "select *, getRoleName(username, portfolio) as role from dialog where idnum > 10000 "
        +" and idnum < 200000 and portfolio = '"+thisUser.getPortfolio()
        +"' and security = 'group' "+ s1
  ...
  * @return HTML formatted string
  */  
  public static String displayMessages(portfolioUser thisUser, 
    ResultSet rs, String thisPageURL) throws SQLException{
  ...
  */
  public static String servletInfo(){
	...
	sbuf.append("<H3>Dialog Help:</H3>\n"
		+"<P>In the left hand collumn under 'Dialog Commands', you see various options.<BR>\n"
		+"<UL>\n"
		+"  <LI><i>List Discussion Topics</i> shows the discussion topics.</LI>\n"
		+"  <LI><i>Create Discussion Topic</i> allows you to create a top level thread.</LI>\n"
		+"  <LI><i>Discussion Quick-view</i> allows you to see all of the messages \n"
		+" in a hierarchy of how they relate to each other.</LI>\n"
		+"</UL>\n");
	
//	sbuf.append("<P>In order to maintain some sense of order within the discussion, normal users are not allowed\n"
//		+" to make top level posts within the <b>public</b> dialog discussion.  If there is a topic you would\n"
//		+" like covered, please contact the portfolio's administrator.\n");
	
	
	sbuf.append("<H3>Dialog info:</H3>\n"
		+"<P>Dialog is a java servlet program for collaborative web discussions.  The program was\n"
		+"developed at the International Institute of Theoretical and Applied Physics\n"
		+" (<a href='http://www.iitap.iastate.edu'>www.iitap.iastate.edu</a>) in support of our\n"
		+" effort to provide collaborative tools.  For more information, see \n"
		+" <a href='http://www.collaborium.org'>www.collaborium.org</a>.\n"

		+"<P>More information about Dailog can be found <a href=\"/jportfolio/info/dialog.html\">here</a>.\n");
	
	sbuf.append("<H3>Servlet Authors:</H3>\n"
		+" <ol>\n<li>Douglas Fils:</li> (<a href='mailto:fils@iastate.edu'>fils@iastate.edu</a>) </li>\n"
		+" <li>Daryl Herzmann:</li> (<a href='mailto:akrherz@iastate.edu'>akrherz@iastate.edu</a>) </li>\n</ol>");
  ...
  */
  public static String actionBox(portfolioUser thisUser, String thisPageURL)
  {
	...
	myBuffer.append("<ul><LI><a href='"+thisPageURL+"?mode=d'>List Discussion Topics.</a></LI>\n");
			
	if ( ! thisUser.getDialogSecurity().equalsIgnoreCase("public") || thisUser.isAdmin() )
			myBuffer.append("  <LI><a href='"+thisPageURL+"?mode=n'>Create Discussion Topic.</a></LI>\n");
			
	myBuffer.append("  <LI><a href=\""+thisPageURL+"?mode=e\">Discussion Quick-view.</a></LI>\n"
			+ "  <LI><a target=\"_new\" "
			+ "href=\""+jlib.httpBase+"/jsp/user/myDialog.jsp\">View All My Posts</a></LI>\n"
			+ "  <LI><a href=\""+thisPageURL+"?mode=i\">Help!</a></LI></ul>\n");    
  ...
  */
  public static String searchBox(String thisPageURL)
  {
	...
	myBuffer.append("<FORM method='post' action='"+thisPageURL+"' name='search'>\n"
  ...
  */
  ...
  {  
	...
	if (!tester)  sbuf.append("No results for this query.<BR>");
  ...
  */
  public static String printOption(String indexVal, String textVal, String lookFor)
  {
  	StringBuffer sbuf = new StringBuffer();
  ...
  * @return HTML Formatted String
  */
  public static String anonPrint(portfolioUser thisUser, ResultSet message, String indentCols, String dataCols){
  	StringBuffer sbuf = new StringBuffer();
	try{
	  ...
	  if (classification == null) classification = "generic";

	    sbuf.append("<tr>\n"
	     +"  <td rowspan=2 colspan=\""+indentCols+"\">&nbsp;</td>\n"
	     +"  <td class=\"postTop\" colspan=\""+dataCols+"\">"
	     +"<font size=+1 class=\""+ classification  +"\">"
	     + message.getString("subject") +" ("+ classification +") </font><br>\n");
	    if (thisUser != null && thisUser.getPortfolio().equals(portfolio) ){
	      sbuf.append("<b>posted by:</b> "+ message.getString("name") +"\n");
	    }
	    sbuf.append("<b>posted on:</b> "+ stringUtils.gmtDate( message.getTimestamp("date") )
	  +"</td>\n"
	  +"</tr>\n"
	  +"<tr>\n"
	  +"  <td class=\"postBottom\" colspan=\""+dataCols+"\" class=\"postBot\">"
  ...
  */
  public static String treeMessages(portfolioUser thisUser, 
    String skipNum, String threadID, String thisPageURL) 
    throws SQLException {  
	...
	if ( thisUser.getDialogSecurity().equalsIgnoreCase("group") ){
		if ( thisUser.isAdmin() ) {
			...
			SQL2 = "SELECT threadid, subject from dialog WHERE "+ threadCount +" "
				+" and portfolio = '"+thisUser.getPortfolio()+"' "
				+" and gID = '"+thisUser.getGroupID()+"' and security = 'group' ";
		}
	}
		...
		if ( thisUser.isAdmin() ) {
			...
			SQL2 = "SELECT threadid, subject from dialog WHERE "
				+" "+threadCount +" and portfolio = '"+thisUser.getPortfolio()+"' "
				+" and security = 'private' ";
		}
		else {
			SQL =  "SELECT * from dialog WHERE "+threadSpec
	...
	while( rs3.next() ) {
		sbuf.append( printOption( rs3.getString("threadid"), rs3.getString("subject"), threadID ) );
	}
  ...
  String commonString = "portfolio = '"+thisUser.getPortfolio()+"' "
    +" and security = '"+thisUser.getDialogSecurity()+"' order by idnum";

  sbuf.append("<p><a href=\"/jportfolio/jsp/user/threadCat.jsp?threadID="+threadID+"\">Printable View of Thread</a>\n");

    //+"  background-repeat: repeat-y;\n"

//    +"  background-color: #FFFFFF;\n"

   sbuf.append("<table bgcolor=\"white\" style=\"background-repeat: repeat-y;\""
     +"background=\"/jportfolio/images/dbar.png\" width=\"100%\"><tr><td>\n");
   sbuf.append("<ul class=\"dqv\">\n");	
   while( threads.next() ) {
      STRidnum = threads.getString("idnum"); 
		sbuf.append( printMessageLine(thisUser, STRidnum, 1, "6", threads, thisPageURL) );
		
		String startNum = STRidnum +"0000";

		threads2 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+" ::numeric"
	+" and idnum < ("+startNum+" +10000)::numeric and "+commonString );
		...
		while(threads2.next()) {
			//if (threads2.isFirst() )
			//sbuf.append("<UL>\n");
			STRidnum = threads2.getString("idnum"); // This shows how to get the value by name
			sbuf.append( printMessageLine(thisUser, STRidnum, 2, "5", threads2, thisPageURL) );
			
			startNum = STRidnum +"0000";
			
			threads3 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+"::numeric "
		+" and idnum < ("+startNum+"+10000)::numeric and "+commonString);
			...
			while( threads3.next() ) {
				...
				sbuf.append( printMessageLine(thisUser, STRidnum, 3, "4", threads3, thisPageURL) );
				
				startNum = STRidnum +"0000";
			
				threads4 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+"::numeric "
			+" and idnum < ("+startNum+"+10000)::numeric and "+commonString );

		        sbuf.append("<ul class=\"dqv\">\n");
				while( threads4.next() ) {
					STRidnum = threads4.getString("idnum"); // This shows how to get the value by name
					sbuf.append( printMessageLine(thisUser, STRidnum, 4, "3", threads4, thisPageURL) );
					
					startNum = STRidnum +"0000";
			
					threads5 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+"::numeric "
						+" and idnum < ("+startNum+"+10000)::numeric and "+commonString );

		            sbuf.append("<ul class=\"dqv\">\n");
					while( threads5.next() ) {
						STRidnum = threads5.getString("idnum"); // This shows how to get the value by name
						sbuf.append( printMessageLine(thisUser, STRidnum, 5, "2", threads5, thisPageURL) );
						
						startNum = STRidnum +"0000";
			
						threads6 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+"::numeric "
							+" and idnum < ("+startNum+"+10000)::numeric and "+commonString );

		                sbuf.append("<ul class=\"dqv\">\n");
						while( threads6.next() ) {
							STRidnum = threads6.getString("idnum"); // This shows how to get the value by name
							sbuf.append( printMessageLine(thisUser, STRidnum, 6, "1", threads6, thisPageURL) );
		 
							startNum = STRidnum +"0000";
							threads7 = dbInterface.callDB("SELECT * from dialog WHERE idnum > "+startNum+"::numeric "
								+" and idnum < ("+startNum+"+10000)::numeric and "+commonString );
		                    sbuf.append("<ul class=\"dqv\">\n");
							while( threads7.next() ){
								STRidnum = threads7.getString("idnum");
								sbuf.append( printMessageLine(thisUser, STRidnum, 6, "1", threads7, thisPageURL) );
							} // while7
							threads7.close();
							sbuf.append("</UL>\n");
						} // while6
  ...
  * @return HTML formatted string
  */
  public static String printMessageLine(portfolioUser thisUser, 
    String STRidnum, int indentCols, 
    String dataCols, ResultSet rs, String thisPageURL) 
    throws SQLException
  {
  	StringBuffer sbuf = new StringBuffer();
	String newStr = " ";
  ...
  sbuf.append("<li><div style=\"background: #FFFFFF\">\n"
    +"<a class=\"boxlink\" "
    +" href=\"javascript: setLayerDisplay('post"+ STRidnum +"');\">+/-</a>\n"

    +"<a "
    +"href=\""+thisPageURL+"?mode=o&idnum="+STRidnum+"\">"+subject+"</a>"
    +" <font class=\""+postType+"\">"
    +" ["+stringUtils.date(date)+"] by "+name+"</font>"+newStr
    +"<div id=\"post"+ STRidnum +"\" style=\"display: none;\">"
    + stringUtils.toBR(rs.getString("body")) +"\n"
    +"<br /><a "
    +" href=\"javascript: setLayerDisplay('post"+ STRidnum +"');\">Hide Post</a></div>\n"
    +"</div></li>\n");
  ...
  *
  */
  public static String inputPost(portfolioMessage myMessage, String thisPageURL)
  {
    StringBuffer sbuf = new StringBuffer();
  
    sbuf.append( jlib.topBox("Posting Preview") );
    sbuf.append("<div class=\"alertBox\"><h3>Alert!</h3>"
     +" Here is a preview of your post.  You can hit the 'back' button\n"
      +" on your browser to edit changes to your post.  Unless you click the 'Finalize \n"
  ...
  */
  public static String inputPostFinal(portfolioMessage myMessage, 
    String thisPageURL) throws ClassNotFoundException, SQLException
  {
    ...
    if (myMessage.getThreadID().equalsIgnoreCase("new")) {
      ResultSet maxth = dbInterface.callDB("select MAX (idnum)+1 as result from dialog"
       +" WHERE idnum > 10000 and idnum < 20000");
		...
		if ( myMessage.getSecurity().equalsIgnoreCase("group") ){
			ResultSet students = dbInterface.callDB("SELECT username from students WHERE portfolio = "
				+" '"+ myMessage.getPortfolio() +"' and gid = "+ myMessage.getGID() +" and "
				+" username != '"+ myMessage.getAuthor() +"' ");
			while (students.next() ) {
				String groupUserID = students.getString("username");
				jlib.updateDB("INSERT into notify(username, portfolio, program, message) "
					+" VALUES( '"+groupUserID+"', '"+ myMessage.getPortfolio() +"', "
					+" '"+notifyBaseURL+"?dialogType=group&mode=r&idnum="+ myMessage.getidnum() +"', "
					+" 'Group Discussion Topic Created: "+ myMessage.getAuthorName() +" posted')");
		}}
		...
		ResultSet maxm = dbInterface.callDB("select MAX(idnum)+1 as result from dialog where idnum > "
			+" "+startNum+"::numeric and idnum < ("+startNum+"+10000)::numeric ");
		maxm.next();
		
		ResultSet test2 = dbInterface.callDB("SELECT security from dialog "
			+" WHERE idnum = '"+myMessage.getThreadID()+"' ");
		test2.next();
		...
		if (newSTRidnum == null) 
			newSTRidnum = myMessage.getidnum()+"0001";  // first post in the thread
		...
		if ( myMessage.getSecurity().equalsIgnoreCase("group") ){
			...
			while (students.next() ) {
				...
				jlib.updateDB("INSERT into notify(username, portfolio, program, message) "
					+" VALUES( '"+groupUserID+"', '"+ myMessage.getPortfolio() +"', "
					+" '"+notifyBaseURL+"?dialogType=group&mode=r&idnum="+ myMessage.getidnum() +"', "
					+" 'Group Discussion Updated: "+ myMessage.getAuthorName() +" posted')");
		}}
		
		jlib.updateDB("insert into notify values('"+myMessage.getReplyAuthor()+"', "
			+" '"+myMessage.getPortfolio()+"',"
			+" '"+notifyBaseURL+"?mode=r&idnum="+ myMessage.getidnum() 
			+"&dialogType="+ myMessage.getSecurity()+"', "
			+" '"+ myMessage.getSecurity() +" Dialog:  "+ myMessage.getAuthorName()+" ')");
  ...
  sbuf.append("<P><a href=\""+thisPageURL+"?mode=r&idnum="+ myMessage.getidnum() +"\">View Post</a>\n");
  ...
  */
  public static String postMessage(String threadid, String STRidnum, String thisPageURL) 
  	throws SQLException, ClassNotFoundException
  {
  ...
  */
  ...
  theBuffer.append("<FORM METHOD='POST' ACTION='"+thisPageURL+"'>\n "
    ...
    + " <P><B>Comments:</B><br> "
    + " <TEXTAREA NAME='body' WRAP='Virtual' ROWS=\"20\" COLS=\"70\"></TEXTAREA>  "
    + "<BR>If you would like to include a link to another webpage, you can enter it "
    + " here:<BR>\n <INPUT TYPE='text' name='link' size='60'>\n");

    theBuffer.append("<P><b>Preview your post:</b>\n"
  ...
  */
  public static String readMore(portfolioUser thisUser, String idNum, 
    String skipNum, String dialogType, String thisPageURL) 
	throws SQLException
  {
	...
	//ResultSet rs2 = dbInterface.callDB("select count(name) as result from dialog "
	//	+"where threadid = " + threadID +" and security = '"+dialogType+"' ");
	ResultSet rs2 = dbInterface.callDB("select count(name) as result from dialog "
		+"where idnum > " + beginID +"0000::numeric and idnum < "+secondOne+"0000::numeric ");
	rs2.next();
	int numMessages = Integer.parseInt( rs2.getString("result") );
	
	if (numMessages > messageBlock ) {
		myBuffer.append("<P align='right'><font class=\"bodyText\">Pages:</font> \n");
		for (int i = 0; i < ( (numMessages-1)/messageBlock +1) ; i++ ) {
			if (!skipNum.equalsIgnoreCase( String.valueOf(i) ) ) {
				myBuffer.append("<a class=\"bodyText\" href='"+thisPageURL+"?mode=r&idnum="+idNum+"&skipNum="+i+"'>"+(i+1)+"</a> &nbsp;\n");
			} else {
				myBuffer.append("<b><font class=\"bodyText\">"+(i+1)+"</font></b> &nbsp;\n");
			}
		}
	} // End of if
...
//	int j = 1;
	...
	ResultSet rs = dbInterface.callDB("select * from dialog where idnum > "+ beginID +"0000::numeric "
		+" and idnum < "+secondOne+"0000::numeric ORDER BY date DESC "
		+" LIMIT "+messageBlock+" OFFSET "+skipMessenges);
	
	myBuffer.append("<font class=\"bodyText\" size=\"+2\">Responses to Active Post:</font><BR>\n");
	if (rs != null){
 ...
 /**
  * threadLeader()
  *	-method that prints out a special box with the current post that is active
  *
  * @param idNum String value for the post we want to display
  * @param identCols int value for the number of columns that this post needs to be shifted over
  * @return String html 
  */

  public static String threadLeader(String idNum, int identCols, String thisPageURL) throws SQLException{
  	StringBuffer sbuf = new StringBuffer();
	...
	sbuf.append("\n\t<!-- Lead Post -->\n<TR>");
	for( int i = identCols; i > 0; i--){ // Loop over the amount of indent we need.
		sbuf.append("<TD>-</TD>");
	}
  ...
  * @return HTML formatted string
  */
  public static String execSearch(portfolioUser thisUser, 
  				String searchStr, 
				String searchCol, 
				String skipNum, 
				String thisPageURL) 
	throws SQLException, ClassNotFoundException
  {
	...
	if ( thisUser.isAdmin() )
		queryStr = "SELECT * from dialog WHERE "+searchCol+" ~* '"+ stringUtils.cleanString(searchStr)+"' "
			+" and security = '"+thisUser.getDialogSecurity()
			+"' and portfolio = '"+thisUser.getPortfolio()+"' ORDER BY date DESC";
	else
	  queryStr = "SELECT * from dialog WHERE "+searchCol+" ~* "
	    +" '"+ stringUtils.cleanString(searchStr)+"' "
			+" and security = 'public' "
			+" and portfolio = '"+thisUser.getPortfolio()
			+"' and gid = "+thisUser.getGroupID()+" ORDER BY date DESC";
  ...
  *
  * @param dialogType value of the current dialogType, so to make the right on selected
  ...
  */
  public static String switchDialogType(String dialogType, String thisPageURL) {
  	...
  	sbuf.append("\n<form method=\"GET\" name=\"switchBox\" action=\""+thisPageURL+"\">\n");
	...
	if ( dialogType.equalsIgnoreCase("public") ) sbuf.append("SELECTED");
	sbuf.append(">Public\n");
	
	sbuf.append("	<option value=\""+thisPageURL+"?dialogType=group\" ");
	if ( dialogType.equalsIgnoreCase("group") ) sbuf.append("SELECTED");
	sbuf.append(">Group\n");
	
	sbuf.append("	<option value=\""+thisPageURL+"?dialogType=private\" ");
	if (dialogType.equalsIgnoreCase("private") ) sbuf.append("SELECTED");
	sbuf.append(">Private\n");
