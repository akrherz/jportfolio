...
public class jdot {
  ...
  public static String bodyItems(portfolioUser thisUser, String skipNum,
    ...
    if (thisUser.getDialogSecurity().equalsIgnoreCase("group"))
      if (thisUser.isAdmin())
        mySQL2 =
            "select *, getRoleName(username, portfolio) as role from dialog where idnum > 10000 "
            + " and idnum < 200000 and portfolio = '" +
            thisUser.getPortfolio() + "' and security = 'group' " + s1 +
  ...
  public static String treeMessages(portfolioUser thisUser, String skipNum,
    ...
    if (threadID != null) {
      ...
      while (threads.next()) {
        ...
        threads2 = dbInterface.callDB("SELECT * from dialog WHERE idnum > " +
                                      startNum + " ::numeric"
                                      + " and idnum < (" + startNum +
        ...
        while (threads2.next()) {
          ...
          threads3 = dbInterface.callDB("SELECT * from dialog WHERE idnum > " +
                                        startNum + "::numeric "
                                        + " and idnum < (" + startNum +
                                        "+10000)::numeric and " + commonString);
  ...
  public static String inputPostFinal(portfolioMessage myMessage,
    ...
    sbuf.append("<P><a href=\"" + thisPageURL +
                "?mode=r&idnum=" + myMessage.getidnum() + "\">View Post</a>\n");
