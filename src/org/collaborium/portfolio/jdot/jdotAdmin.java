...
public class jdotAdmin extends HttpServlet {
        ...
        throws ServletException, IOException
  {
    ...
    case 'g':
      if (selectedUserID == null)
        ...
        pageContent.append( userOptions(thisUser.getPortfolio(), selectedUserID) );
    break;
		case 't':
  ...
  */
  public String printIntro() {
	...
	sbuf.append("<P>This page allows you to administer various components of the Dialog.  You can view stats for the dialog \n"
...
//  String dateDue   = (String)req.getParameter("duedate");
	...
	+" '"+ portbase +"?dialogType=public&mode=r&idnum="+newSTRidnum+"' ) ");
		}
		else // Otherwise just get a listing of students in the group
			students = dbInterface.callDB("SELECT username from students WHERE portfolio = "
				+" '"+thisUser.getPortfolio()+"' and gid = "+groupID+" ");
		
		// Put in the DB
		jlib.updateDB("INSERT into dialog(username, name, subject, threadid, idnum, "
			+" portfolio, security, gid, body, type, expires) values ('"+thisUser.getUserID()+"', "
			+" '"+ thisUser.getRealName() +"', '"+subject+"' "
			+" , '"+newSTRidnum+"', '"+newSTRidnum+"', '"+ thisUser.getPortfolio() +"', '"+security+"', '"+groupID+"' "
		...
		while (students.next() ) {
			...
			jlib.updateDB("INSERT into notify(username, portfolio, program, message) "
				...
				+ "'"+ portbase +"?dialogType="+security+"&mode=r&idnum="+newSTRidnum+"', "
				+" 'Dialog Assigned:')");
  ...
  */
  ...
  ResultSet maxth = dbInterface.callDB("select MAX (idnum)+1 as result1, "
  ...
  if (score1 != null && score1.length() != 0){
    ...
    +" '"+notifyBaseURL+"?dialogType=private&mode=r&idnum="+newSTRidnum1+"', "
    +" 'Evaluation Made:')");
  }
  ...
  if (score2 != null && score2.length() != 0){			
   ...
   jlib.updateDB("INSERT into notify(username, portfolio, program, message) "   
   +" VALUES( '"+selectedUserID+"', '"+ thisUser.getPortfolio()+"'," 
   +" '"+notifyBaseURL+"?dialogType=private&mode=r&idnum="+newSTRidnum2+"', " 
   +" 'Ethical Q Eval:')");
  }
  ...
  */
  public String createPortfolioAssign(portfolioUser thisUser) {
    ...
    sbuf.append("<form method='POST' action='"+thisPageURL+"' name='motd'>\n");
    sbuf.append("<input type='hidden' name='mode' value='e'>\n"); 
    sbuf.append("<font class=\"instructions\">\n"
  ...
  */
  public String gradeDialog(String userID, String portfolio) {
    ...
    String EDATE = "2007-03-26";
 	try{
        // Stats on user posts
		ResultSet rs = dbInterface.callDB("SELECT count(username) as count, "
		...
		ResultSet rs2 = dbInterface.callDB("SELECT idnum from dialog "
		...
		ResultSet rs4 = dbInterface.callDB("SELECT count(username) as count "
		...
		while ( rs2.next() ) {
			...
			ResultSet rs3 = dbInterface.callDB("SELECT "
             + totalCount +" + count(username) as count "
		     +"  from dialog WHERE username != '"+userID+"' "
  ...
  if (rs.next() ){
    sbuf.append("<table>\n"
      +"<tr><th>Total Posts</th> <td>"+ rs.getString("count") +"</td></tr>\n"
  ...
  sbuf.append("<P><B>Post Your Evaluations:</B><BR>\n"
	...
	+"<input type=\"hidden\" value=\""+userID+"\" name=\"selectedUser\">\n"
  ...
  */
  public String userOptions(String portfolio, String selectedUserID) {
	...
	sbuf.append("<font class=\"instructions\"><blockquote>This page allows you to view a certain users dialog\n"
		+" and assign some sort of grade to the user.</blockquote></font>\n");
		
	sbuf.append("<P>View All User Dialog:<BR>");
	sbuf.append("<a href=\""+jlib.httpBase+"/jsp/admin/printDialog.jsp?blockID=1&selectedUser="+jlib.cleanGetString(selectedUserID)+"\" "
			+" target=\"_new\">Block 1</a>\n"
		+"<BR><a href=\""+jlib.httpBase+"/jsp/admin/printDialog.jsp?blockID=2&selectedUser="+jlib.cleanGetString(selectedUserID)+"\" "
			+" target=\"_new\">Block 2</a>\n"
		+"<BR><a href=\""+jlib.httpBase+"/jsp/admin/printDialog.jsp?blockID=3&selectedUser="+jlib.cleanGetString(selectedUserID)+"\" "
  ...
  public String dailyPosts(String portfolio){
	...
	sbuf.append("<blockquote><font color=\"green\">Here is a daily summary of total \n"
	...
	try{
		ResultSet rs = dbInterface.callDB("SELECT count(username) as posts, date(date) , avg(length(body)) as len from dialog "
		...
		while( rs.next() ){
			...
			sbuf.append("  <TR><TD>"+strDate+"</TD><TD>"+posts+"</TD><TD>"+len+"</TD></TR>\n");
		}
  ...
  */
  public String statsOptions() {
	...
	sbuf.append( jlib.topBox("Available Statistics:") );
 	...
 	sbuf.append("<UL>\n"
		+"  <LI><a href=\""+thisPageURL+"?mode=t\">Daily Posts</a></LI>\n"
		+"</UL>\n");
  ...
  public String adminCommands()
  {
	...
	myBuffer.append("<ul><li><a class=\"commands\" href='"+thisPageURL+"?mode=s'>Stats</a></li>\n"
		+" <li><a class=\"commands\" href='"+thisPageURL+"?mode=e'>Assign A Group Dialog</a></li>\n"
		+" <li><a class=\"commands\" href='"+thisPageURL+"?mode=f'>Assign A Portfolio Dialog</a></li>\n"
		+" <li><a class=\"commands\" href='"+thisPageURL+"?mode=g'>Grade Dialog</a></li></ul>\n");
