...
public class jquiz extends HttpServlet {
  ...
  */
  ...
  switch ( callMethod.charAt(0) ) {
    ...
    default:
     ...
     pageContent.append("<P>"+ takableQuizzes(thisUser, studentGrades, false, thisPageURL) );
     break;
  } // End of switch()
  ...
  */
  ...
  sideContent.append( sideBar(thisUser, studentGrades) );	
  out.println( jlib.header(thisUser, TITLE, "Quiz") );	
  out.println( jlib.makePage( sideContent.toString(), 
 ...
 public static String introduction() {
	...
	sbuf.append( jlib.topBox("Information:") );
 	...
 	sbuf.append("<font class=\"Instructions\"><blockquote>This page contains quiz links and \n"
  ...
  public static String quizBoxHelp() {
	...
	sbuf.append( jlib.topBox("Quiz Box Info:") );
 	...
 	sbuf.append("<font color=\"green\"><blockquote>The box on the left hand side lists out quizzes that are\n"
  ...
  */
  public static String reviewQuizzes(portfolioUser thisUser) 
  {
    ...
    try{
      ...
      while( rs.next() ) {
       sbuf.append("<TR><TD> "
         +"<a href=\""+thisPageURL+"?mode=p&qid="+rs.getString("qid")+"\">\n"
  ...
  */
  public static String inputQuiz(portfolioUser thisUser, 
    HttpServletRequest request, gradebook studentGrades)
  {
  ...
  try{
	...
	// Avoid any null pointers
  	...
  	portfolioQuiz thisQuiz = new portfolioQuiz( quizID, "yes" ); 	
		 ...
		 */
		ResultSet rs6 = jlib.callDB("SELECT attempt +1 as att from quizattempts WHERE"
		...
		if ( thisQuiz.goodAttempt( thisAttempt ) ){ // Allowed
			...
			jlib.updateDB("INSERT into quizattempts(qid, question1, question2, question3,"
   				+" userid, portfolio, attempt) VALUES"
   				+" ("+quizID+", "
   				+" '"+question1+"', '"+question2+"', '"+question3+"', "
   				+" '"+ thisUser.getUserID()+"', '"+ thisUser.getPortfolio()+"', "+thisAttempt+")");
			sbuf.append("Your quiz attempt has successfully been saved.");
		
			sbuf.append("<H3>Quiz Grade:</H3>");
		
	
			String myRight = thisQuiz.numberRight(question1, question2, question3);
			 ...
			 */
   			if (! allowedAttempts.equalsIgnoreCase("1") ) {
				...
				if (numQs.equalsIgnoreCase(myRight) ){
   				...
   				sbuf.append("<BR>You got "+myRight+" out of "+numQs+" questions right!\n");
				
			} else{
				sbuf.append("Your quiz has been saved.  You can find out your grade after the quiz has \n"
					+" expired.<BR>\n");
			}
			...
			// Those bastards
		//	jlib.updateDB("INSERT into abuse values('"+ thisUser.getUserID()+"')");
		
			sbuf.append("<font class=\"warn\">Access Warning:</font> This quiz has been closed.  This quiz \n"
  ...
  if (body.length() > 4){
    ...
    */
    ...
    ResultSet maxth = jlib.callDB("select MAX (idnum)+1 as result from dialog"
...
//	  myMessage.setReplyAuthor( ne);
    ...
    for (int i=0; i < myVect.size(); i++){
      ...
      dbInterface.updateDB("insert into notify(username, portfolio, program "
	+", message) values('"+ thisUserID +"', '"+thisUser.getPortfolio()+"' "
	+", '"+notifyBaseURL2+"?dialogType=private&mode=r&idnum="+ myMessage.getidnum() +"' "
	+", 'Quiz "+thisQuizName+" Response: "+ thisUser.getRealName() +"')");
    ...
    HttpServletRequest request)
  {
    ...
    try { // Do the database insert here
      if (body.length() > 4){
        ...
        ResultSet portHome = dbInterface.callDB("SELECT porthome "
         +" from portfolios WHERE portfolio = '"+ thisUser.getPortfolio() +"' "
        ...
        ResultSet maxth = jlib.callDB("select MAX (idnum)+1 as result "
...
//	  myMessage.setReplyAuthor( ne);
        ...
        for (int i=0; i < myVect.size(); i++){
          String thisUserID = (String)myVect.get(i);
          dbInterface.updateDB("insert into notify(username, portfolio, "
           +" program, message) values('"+ thisUserID +"', "
           +" '"+thisUser.getPortfolio()+"' , '"+notifyBaseURL2+"?dialogType=private&mode=r&idnum="+ myMessage.getidnum() +"' "
	   +",'Post Quiz "+thisQuizName+" Response: "+ thisUser.getRealName() +"')");
    ...
    sbuf.append("<p>Your dialog post has been inserted in the database.  You "
  ...
  */
  public static String takeQuiz(portfolioUser thisUser, 
    String quizID, String thisPageURL) {
	...
	sbuf.append( thisQuiz.printQ3() );
        ...
        sbuf.append("<p><b>Quiz Feedback:</b><br>\n"
  ...
  * @return HTML string 
  */  
  public static String sideBar(portfolioUser thisUser, gradebook studentGrades)
  {
	...
	theBuffer.append("<P>\n");
	theBuffer.append( takableQuizzes(thisUser, studentGrades, true, thisPageURL) );
	theBuffer.append("<P>\n");
	if ( thisUser.isAdmin() ) 
  ...
  */
  public static String listQuizzes(portfolioUser thisUser)
  {
	...
	ResultSet rs = null;
	try {
		rs = jlib.callDB("SELECT quizes.qname, quizes.quiznum, getQuestionText(question1id) as q1,"
		...
		+" quizattempts.userID = '"+ thisUser.getUserID() +"' and quizes.quiznum = quizattempts.qid "
		+" ORDER by entered DESC LIMIT 3");
		while ( rs.next() ) {
			myBuffer.append("<P><a href=\""+thisPageURL+"?mode=p&qid="+rs.getString("quiznum")+"\">"+rs.getString("qname")+"</a><BR>\n"
				+" <blockquote>\n"
 ...
 /**
  * Method to print out a nice display of the quiz and what the student answered
  * 
  ...
  */
  public static String viewQuiz(portfolioUser thisUser, 
    gradebook myGrades, String qid)
  {
    StringBuffer sbuf = new StringBuffer();
    ...
    sbuf.append("<blockquote><font color=\"green\">Here is a review of a past"
	...
	String quizScore = myGrades.getScoreElement("jquiz", 
           qid, thisQuiz.getQuizName() );
        ...
        ResultSet myQuizAttempt = jlib.callDB("SELECT * from quizattempts "
         +" WHERE userid = '"+ thisUser.getUserID()+"' "
         +" and portfolio = '"+ thisUser.getPortfolio()+"' "
         +" and qid = "+ qid+" ");
       myQuizAttempt.next();
    ...
    sbuf.append("<p><b>Post response:</b>  Perhaps you would like to "
   ...
   sbuf.append("<FORM METHOD='POST' ACTION='"+thisPageURL+"'>\n"
    ...
    sbuf.append("<p><TEXTAREA NAME='body' WRAP='Virtual' ROWS=\"8\" COLS=\"50\"></TEXTAREA>\n");
  ...
  public static String printOption(String qNum, String textVal, String answer, String myAnswerID)
  {
      StringBuffer sbuf = new StringBuffer();
      System.err.println("Question num -> "+qNum+"  Answer -> "+answer+"  myAnswer -> "+myAnswerID+" \n");
  ...
  */
  public static String scoreBox(gradebook studentGrades)
  {
	...
	myBuffer.append("<TABLE BORDER='0'>\n"
		+ "<TR><TD>Cumulative Points:</TD> <TD>"+studentGrades.totalScores()+"</TD></TR>\n"
		+ "<TR><TD>Score on Quizzes:</TD> <TD>"+studentGrades.appScores("jquiz")+"</TD></TR>\n"
		+ "</TABLE>\n");
  ...
  */   
  public static String takableQuizzes( portfolioUser thisUser, gradebook studentGrades, 
		boolean isShort, String thisPageURL ) {
  ...
  if (rs != null) {  
   try{
	...
	if (isShort && qName.length() > 11)
	  ...
	  +" and qid = "+qID+" ");
   	if (rs2.next() ) {
   	  ...
   	  if (java.lang.Integer.parseInt(myAttempts) == -1) // Student got them all right!
		sbuf.append("<TR><TH>"+qName+"</TH><TD>Done</TD>\n");
	  else if (java.lang.Integer.parseInt(allowedAttempts) == 0 || java.lang.Integer.parseInt(myAttempts) < java.lang.Integer.parseInt(allowedAttempts) ) 
	  	sbuf.append("<TR><TH><a href='"+thisPageURL+"?mode=t&qid="+qID+"'> "+qName+"</a></TH>\n"
 			+"<TD>"+myAttempts+" - "+allowedAttempts+"</TD>\n");					else
  		sbuf.append("<TR><TH>"+qName+"</TH><TD>Done</TD>\n");
  	}	
	else {
	  sbuf.append("<TR><TH><a href='"+thisPageURL+"?mode=t&qid="+qID+"'> "+qName+"</a></TH>"
	...
	rs2.close();
	sbuf.append("<TH>"+ studentGrades.getScoreElement("jquiz", qID, scoreName) +"</TH></TR>\n");
  ...
  sbuf.append("</TABLE>\n");
  sbuf.append("<P align=\"right\"><a href=\""+thisPageURL+"?mode=h\">Help</a>\n");
  sbuf.append( jlib.botBox() );
  ...
  public static String commands(portfolioUser thisUser)
  {
	...
	myBuffer.append("<LI><a class=\"commands\" href='"+thisPageURL+"?mode=r'>Review all my quizes</a></LI>\n");    
	myBuffer.append( jlib.botBox() );
  ...
  */ 
  public static String intro()
  {
	...
	myBuffer.append("This is the Quiz servlet for the Portfolio pacakge.  It is designed to allow "
