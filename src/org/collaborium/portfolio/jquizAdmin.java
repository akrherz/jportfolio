...
public class jquizAdmin extends HttpServlet {
  ...
  */
  public String whichQuiz(portfolioUser thisUser){
	...
	sbuf.append("<FORM method=\"POST\" action=\""+thisPageURL+"\" name=\"quizNum\">\n"
		+"<input type=\"hidden\" name=\"mode\" value=\"l\">\n"
		+"<SELECT name=\"quizNum\">\n");
	
	try{
		ResultSet rs = jlib.callDB("SELECT * from quizes WHERE "
  ...
  */
  public String quizResults(portfolioUser thisUser, String quizName){
	...
	sbuf.append("<blockquote><font color=\"green\">Here are listed the quiz results.</font></blockquote>\n");
		
	sbuf.append("<H3>Quiz: "+quizName+"\n");
	
	try{
	
		ResultSet rs = jlib.callDB("SELECT score, getUserName(userid) as name from scores s "
			+" WHERE s.portfolio = '"+thisUser.getPortfolio()+"' "
			+" and s.assign = '"+quizName+"' ");
		ResultSet rs2 = jlib.callDB("SELECT AVG(score) as average_score from scores s "
			+" WHERE s.portfolio = '"+ thisUser.getPortfolio()+"' "
			+" and s.assign = '"+quizName+"' ");
		rs2.next();
  ...
  public String listQuestions(portfolioUser thisUser){
  	...
  	sbuf.append("<P><blockquote><font color=\"green\">With this dialog you are able to edit a question that\n"
	...
	sbuf.append("<FORM METHOD=\"POST\" ACTION=\""+thisPageURL+"\" name=\"EDIT\">\n"
   ...
   public String editQuestionDialog(portfolioUser thisUser, HttpServletRequest req){
  	StringBuffer sbuf = new StringBuffer();
	...
	sbuf.append( jlib.topBox("Edit Question?") );
  	...
  	try{
		ResultSet rs = jlib.callDB("SELECT * from questions "
		  +" WHERE qid = '"+qid+"' and portfolio = '"+ thisUser.getPortfolio() +"' ");
		...
		sbuf.append("<form method='POST' action='"+thisPageURL+"'>\n"
			...
			+"		<input type='text' size='60' name='question' value=\""+question+"\"></TD>\n"
			+"<TD></TD>"
			+" </TR>\n\n"
			...
			+ mkAnswerOption("g", optiong, answer)
	  		+ mkAnswerOption("h", optionh, answer)
	  		+" </TABLE>\n"
  ...
  */
  	...
  	throws myException
  {
	...
	String question = stringUtils.cleanString( (String)req.getParameter("question") );
	String answer = (String)req.getParameter("answer");
	String qid = (String)req.getParameter("qid");
	String optiona = stringUtils.cleanString( (String)req.getParameter("optiona") );
	String optionb = stringUtils.cleanString( (String)req.getParameter("optionb") );
	String optionc = stringUtils.cleanString( (String)req.getParameter("optionc") );
	String optiond = stringUtils.cleanString( (String)req.getParameter("optiond") );
	String optione = stringUtils.cleanString( (String)req.getParameter("optione") );
	String optionf = stringUtils.cleanString( (String)req.getParameter("optionf") );	
	String optiong = stringUtils.cleanString( (String)req.getParameter("optiong") );
	String optionh = stringUtils.cleanString( (String)req.getParameter("optionh") );
	...
	if (qid != null) {
		...
		jlib.updateDB("UPDATE questions SET question = '"+question+"', "
			...
			+" optionf = '"+optionf+"', "
			+" optiong = '"+optiong+"', "
			+" optionh = '"+optionh+"', answer = '"+answer+"' WHERE qid = "+qid+" ");
		...
		try{
		jlib.updateDB("INSERT into questions(portfolio, question, optiona, optionb, optionc, optiond,"
			+" optione, optionf, optiong, optionh, answer)"
			+" VALUES ('"+ thisUser.getPortfolio()+"', '"+question+"', '"+optiona+"' , '"+optionb+"', '"+optionc+"', "
			+" '"+optiond+"', '"+optione+"', '"+optionf+"', '"+optiong+"', '"+optionh+"', '"+answer+"') ");
 ...
 */
  	...
  	throws myException
  {
	...
	if ( qName.equalsIgnoreCase("") ) qName = "Unnamed Quiz";
	if ( question1.equalsIgnoreCase("null") ) question1 = "0";
	if ( question2.equalsIgnoreCase("null") ) question2 = "0";
	if ( question3.equalsIgnoreCase("null") ) question3 = "0";
	if ( attempt.equalsIgnoreCase("") ) attempt = "0";
	...
	try{
		// We can't have duplicate quiz names yet, since I messed up on the scores table.
		ResultSet tester = jlib.callDB("SELECT quiznum from quizes "
			+" WHERE qname = '"+qName+"' and portfolio = '"+thisUser.getPortfolio() +"'");
		if (! tester.next() ) {
	
			// Create Quiz Entry
			jlib.updateDB("INSERT into quizes(qname, portfolio, question1, question2, "
				+" question3, startDate, stopDate, attempts, topicid)"
				+" VALUES ('"+qName+"', '"+thisUser.getPortfolio()+"', "+question1+", '"+question2+"' ,"
				+"  "+question3+", '"+startDate+"', '"+endDate+"', "+attempt+", '"+topicid+"') ");
			// Figure out what quiznum this quiz was assigned
			ResultSet test2 = jlib.callDB("SELECT quiznum from quizes WHERE "
			 	+" qname = '"+qName+"' and portfolio = '"+thisUser.getPortfolio() +"'");
			test2.next();
			String thisQuizNum = test2.getString("quiznum");
	
			// Create Calendar Entries
			jlib.updateDB("INSERT into calendar(valid, portfolio, description, url) VALUES "
				+" ('"+startDate+"', '"+thisUser.getPortfolio()+"', 'Quiz "+qName+" Available', "
				+" '"+notifyBaseURL+"?mode=t&qid="+thisQuizNum+"')");
			jlib.updateDB("INSERT into calendar(valid, portfolio, description) VALUES "
				+" ('"+endDate+"', '"+thisUser.getPortfolio()+"', 'Quiz "+qName+" Expires')");
	
			sbuf.append("Input Quiz was successful!");
		} else {
			sbuf.append("<P>A quiz by this name allready exists, please use a different name.\n");
		}
  ...
  public String mkIntroduction() {
	...
	sbuf.append( jlib.topBox("Jquiz Admin Instructions:") );
 	...
 	sbuf.append("<P>The purpose of jquizAdmin is to allow instructor the ability to edit and manipulate quizzes for the portfolio "
  ...
  */
  public String newQuiz(portfolioUser thisUser) {
	...
	sbuf.append( "<blockquote><font color=\"green\">This dialog also you to combine previously created questions into\n"
	...
	sbuf.append(" <P>Select Question 1:<BR>\n"
	  + listQuestions(thisUser , "1")
 ...
 public String listQuizzes(portfolioUser thisUser) {
  	...
  	sbuf.append("<TR><TH>Quiz Name:</TH><TH>Start Date:</TH><TH>End Date:</TH></TR>\n");
	ResultSet rs = null;
  	try {
  ...
  public String listQuestions(portfolioUser thisUser, String id) {
  	...
  	try {
  		rs = jlib.callDB("SELECT qid, question from questions "
			+" WHERE portfolio IN "+ thisUser.myPortfolio.getPList() +" "
			+" ORDER by question");
		while( rs.next() ){
			try{
				sbuf.append("<option value='"+rs.getString("qid")+"'>"+ rs.getString("question").substring(0,70) +"\n");
			} catch(Exception ex) {
				sbuf.append("<option value='"+rs.getString("qid")+"'>"+ rs.getString("question") +"\n");
			}
		}
  ...
  */
  public String adminCommands()
  {
	...
	myBuffer.append("<ul><li><a class=\"commands\" href='"+thisPageURL+"?mode=q'>New Question</a></li>\n"
		+"   <li><a class=\"commands\" href='"+thisPageURL+"?mode=t'>Edit Question</a></li>\n"
		+"   <li><a class=\"commands\" href='"+thisPageURL+"?mode=u'>Create A Quiz</a></li>\n"
		+"   <li><a class=\"commands\" href='"+thisPageURL+"?mode=v'>List Quizzes</a></li>\n"
		+"   <li><a class=\"commands\" href='"+thisPageURL+"?mode=l'>Quiz Results</a></li></ul>");    
  ...
  */
  ...
  public String mkAnswerOption(String optionAlpha, String optionVal, String answer) {
  	StringBuffer sbuf = new StringBuffer();
  
  	sbuf.append("\n<TR><TH align=\"left\">Option "+optionAlpha+":<BR>\n"
		+"<input type=\"text\" size=\"60\" name=\"option"+optionAlpha+"\" value=\""+optionVal+"\"></TH>\n");
