...
public class jportfolioAdmin extends HttpServlet {
  ...
  */
  public String selectApps(portfolioUser thisUser, String thisPageURL) {
	...
	sbuf.append("<font class=\"instructions\">You can restrict which elements are usable by \n"
  ...
  */
  public String changeApps(portfolioUser thisUser, HttpServletRequest request) {
	...
	String usesChat = (String)request.getParameter("usesChat");
  	try{
		dbInterface.updateDB("INSERT into appregistry VALUES('"+ thisUser.getPortfolio() +"') ");
		dbInterface.updateDB("UPDATE appregistry SET use_calendar = '"+ usesCalendar +"' "
  ...
  public String listStudentGrades(String portfolio)
  {
	...
	sbuf.append("<TABLE>\n");
	try{
		ResultSet rs = dbInterface.callDB("SELECT sum(score) as tot_score, getUserName(userid) as name from scores s "
			+" WHERE s.portfolio = '"+portfolio+"' GROUP by s.userid");
		while ( rs.next() ){
			sbuf.append("<TR><TD>"+rs.getString("name") +"</TD><TD>"+ rs.getString("tot_score") +"</TD></TR>\n");
		}
  ...
  */
  public String listStudentGrades(portfolioUser thisUser, String selectedUserID)
  {
  ...
  public String addAdminDialog(String portfolio) 
  {
  	...
  	sbuf.append("<P><blockquote><font color=\"green\">With this form you can knight somebody else to share\n"
	...
	try {
		ResultSet rs = dbInterface.callDB("SELECT * from admins WHERE portfolio = '"+portfolio+"' ");
		while( rs.next() ) {
	...
	throws ServletException, IOException 
  {
	...
	switch ( callMethod.charAt(0) ){	
        ...
        case 'r':
          ...
          break;
		case 'c':
			pageContent.append( enterCalendarElement(request, thisUser.getPortfolio()) );
		break;
		case 'g':
		...
		case 'n': // Send email
			...
			pageContent.append( portfolioUtils.emailPortfolio( message, subject, thisUser ) );
  ...
  */
  public String postNewAdmin(HttpServletRequest req, portfolioUser thisUser) {
	...
	sbuf.append("You are adding user <i>"+newAdmin+"</i> as an administrator"
		+" to portfolio <i>"+portfolio+"</i>\n");
	try {
		jlib.updateDB("DELETE from admins WHERE admin = '"+newAdmin+"' and portfolio = '"+portfolio+"' ");
		jlib.updateDB("INSERT into admins(portfolio, admin) VALUES ('"+portfolio+"', '"+newAdmin+"') ");
	} catch( Exception ex) {
 ...
 public String postMOTD( HttpServletRequest req, portfolioUser thisUser) {
	...
	System.err.println(motd);
 	if (motd == null) motd = "Cowardly MOTD";
	...
	String sendAsEmail = req.getParameter("sendEmail");
 	...
 	try {
		...
		ResultSet newMotd = dbInterface.callDB("SELECT last_value+1 as newmotd from motd_id_seq");
		newMotd.next();
		String newValue = newMotd.getString("newmotd");
		
 		jlib.updateDB("INSERT into motd(portfolio, body) "
			+" values('"+portfolio+"', '"+jlib.cleanString(motd)+"')");
		
		// Lets notify everybody in the portfolio that they have a MOTD to look at.
		...
		while (students.next() ) {
			...
			jlib.updateDB("INSERT into notify(username, portfolio, program, message) "
				...
				+" '"+jlib.httpBase+"/jsp/user/printMotd.jsp?idnum="+newValue+"', "
		...
		jlib.updateDB("INSERT into calendar(portfolio, description, url) VALUES "
			+" ('"+portfolio+"', 'MOTD Post', '"+jlib.httpBase+"/jsp/user/printMotd.jsp?idnum="+newValue+"')");
		
		if ( sendAsEmail != null ) {
			System.err.println("I am sending the Portfolio An Email");
			sbuf.append( portfolioUtils.emailPortfolio( motd, "MOTD Posted", thisUser ) );
		}
  ...
  */
  public String addCalendarElement() {
    StringBuffer sbuf = new StringBuffer();
     ...
     sbuf.append("<font class=\"instructions\">With this dialog, "
  ...
  */
  public String enterCalendarElement(HttpServletRequest req, String portfolio) {
	...
	String description = stringUtils.cleanString( req.getParameter("description") );
	String validDate = (String)req.getParameter("validDate");
	String thisURL = (String)req.getParameter("URL");
	String sqlString = "";
	if (thisURL == null)  sqlString = "INSERT into calendar(valid, description, portfolio) VALUES "
			+" ('"+validDate+"', '"+description+"', '"+portfolio+"') ";
	else
		sqlString = "INSERT into calendar(valid, description, portfolio, url) VALUES "
			+" ('"+validDate+"', '"+description+"', '"+portfolio+"', '"+thisURL+"') ";
  ...
  public String listStudents(String portfolio) {
 	...
 	try{
 		rs = dbInterface.callDB("SELECT u.fname, u.lname from students s, users u"
			+" WHERE s.portfolio = '"+portfolio+"' and s.username = u.username");
 		while (rs.next() ) {
 			sbuf.append("<P>"+rs.getString("fname")+" "+rs.getString("lname")+" ");
  ...
  */
  public String editMOTD() {
	...
	sbuf.append("<blockquote><font color=\"green\">The message of the day appears when the portfolio\n"
	...
	sbuf.append("<P>Enter Message:<BR>"
  ...
  public String adminCommands()
  {
	...
	myBuffer.append("<ul><li><a href='"+thisPageURL+"?mode=m'>Post MOTD</a></li>\n"
     +"<li><a href='"+thisPageURL+"?mode=r'>Assign User Roles</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=s'>Select Portfolio Elements</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=g'>Edit Students Group IDs</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=l'>List Students</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=b'>Add Administrator</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=c'>Make a Calendar Entry</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=h'>List Sum Grades</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=i'>List Student Grades</a></li>\n"
		+"   <li><a href='"+thisPageURL+"?mode=n'>Send Portfolio Email</a></li></ul>\n");    
  ...
  public String editGroupIDs(String portfolio){
	...
	try {
		students = dbInterface.callDB("SELECT *, getUserName(username) as realname from students "
			+" WHERE portfolio = '"+portfolio+"' and gid = -99 ");
		while ( students.next() ) {
			userID = students.getString("username");
			realname = students.getString("realname");
			sbuf.append("<P>"+realname+"<input type='text' name='"+userID+"' size='6' MAXLENGTH='4'>");
		}
	} catch(Exception ex) {
	...
	// Loop over the rest of the students to make a printout of allready assigned values
  ...
  */
  public String assignRolesForm(portfolioUser thisUser)
  {
    ...
    sbuf.append("<form method=\"POST\" name=\"assign\" action=\"/jportfolio/servlet/jportfolioAdmin\"><input type=\"hidden\" value=\"r\" name=\"mode\">\n");
    ...
    while (rs.next())
    {
      sbuf.append("<tr><td>"+ rs.getString("realname") +"</td>"
       ...
       +"<td><input type=\"text\" name=\""+ rs.getString("username") +"\" "
       +"size=\"4\" value=\""+ rs.getString("id") +"\"></td></tr>\n");
    }
    } catch(Exception ex){
      System.err.println( ex );
    }
      ...
      try {
        if (!userID.equalsIgnoreCase("mode") && ! gID.equals("") ) {
          jlib.updateDB("UPDATE students SET gid = '"+gID+"' "
           +"WHERE username = '"+userID+"' and portfolio = '"+portfolio+"'");
        }
      } catch (Exception ex){
        System.err.println( ex );
      }
  ...
  public String intro()
  {
	...
	myBuffer.append("<font size=+1>This is the portfolio manager."
  ...
  public String adminSideBar(portfolioUser thisUser) {
	...
	theBuffer.append("<P>");
	theBuffer.append( jlib.currentUsers(thisUser.getPortfolio(), thisUser.getUserID() ) );
	theBuffer.append("<P>");
  ...
  public String classPortfolio(String portfolio) 
  {
  	...
  	try {
  		rs = dbInterface.callDB("SELECT * from motd "
			+" WHERE portfolio = '"+portfolio+"' ORDER by issue DESC LIMIT 1");
  		if ( rs.next() )
	...
	if (motd == null) motd = "Message not found";
