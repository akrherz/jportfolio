  ...
  */
  public String printInfo(String requestedUser, Boolean writePerm){
  	...
  	sbuf.append("	<table cellpadding=0 cellspacing=0 border=1 width=200>\n"
		...
		+"	<BR>\n"
		+"	<CENTER><img src=\"/jportfolio/FILES/"+ requestedUser +"/me.gif\"></CENTER>\n"
		+"	<BR>\n");

	if ( writePerm.booleanValue() )
  		sbuf.append("	<a href=\"/jportfolio/jsp/user/customize/editPict.jsp\">Edit</a>\n");
	...
	try{
		ResultSet rs = dbInterface.callDB("select s.portfolio, p.name, "
		...
		while ( rs.next() ){
			sbuf.append("<li><a "
	+" href=\"/jportfolio/servlet/jportfolio?portfolio="+ rs.getString("portfolio") +"\">"
	...
	try{
		ResultSet rs = dbInterface.callDB("SELECT getUserName(username) as realn, * from users"
		+" WHERE username = '"+ requestedUser +"' ");
	
		if (rs.next() ) {
		  sbuf.append("<TABLE>\n");
		  sbuf.append("<TR><TD>Real Name:</TD><TD> "+ rs.getString("realn") +"</TD></TR>\n"
			+"<TR><TD>Email Address:</TD><TD> "+ rs.getString("email") +"</TD></TR>\n");
		  sbuf.append("</TABLE>\n");
	//	  if ( writePerm.booleanValue() )
	//	  	sbuf.append("	<a href=\"/jportfolio/jsp/user/customize/editInfo.jsp\">Edit</a>\n");
		  
		}
	
		ResultSet rs2 = dbInterface.callDB("SELECT body from biosketch WHERE "
