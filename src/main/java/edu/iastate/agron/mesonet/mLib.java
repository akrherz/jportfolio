...
public class mLib {
  ...
  public static String stationSelect(String portfolio) throws SQLException {
    ...
    ResultSet rs =
        dbInterface.callDBWithParameters(
            "SELECT * from iem_sites WHERE " + " portfolio = ? ORDER by s_name ASC",
            Arrays.asList(portfolio));
    sbuf.append("<select name=\"s_mid\">\n");
    while (rs.next()) {
      sbuf.append(
          "  <option value=\"" + rs.getString("s_mid") + "\">" + rs.getString("s_name") + "\n");
    } // End of rs.next()
    sbuf.append("</select>\n");
   ...
   * @return HTML formated string
   */
  public static String stationSelectMulti(String portfolio) throws SQLException {
    StringBuffer sbuf = new StringBuffer();

    ResultSet rs =
        dbInterface.callDBWithParameters(
            "SELECT * from iem_sites WHERE " + " portfolio = ? ", Arrays.asList(portfolio));
   ...
   * @return HTML formated string
   */
  public static String listContacts(String mid, String portfolio) throws SQLException {
    StringBuffer sbuf = new StringBuffer();
    sbuf.append(
        "<table width=\"100%\">\n"
    ...
    ResultSet rs =
        dbInterface.callDBWithParameters(
            "SELECT * from iem_site_contacts WHERE " + " s_mid = ? and portfolio = ? ",
