...
public class jportfolioAdmin extends HttpServlet {
  ...
  public String selectApps(portfolioUser thisUser, String thisPageURL) {
    ...
    sbuf.append(
        "<font class=\"instructions\">You can restrict which "
    ...
    if (thisUser.usesQuiz) {
      sbuf.append(
          "<TD><input type=\"radio\" value=\"yes\" name=\"usesQuiz\" "
              + "CHECKED>yes\n"
              + "</TD><TD><input type=\"radio\" value=\"no\" "
  ...
  public String changeApps(portfolioUser thisUser, HttpServletRequest request) {
    ...
    try {
      ...
      dbInterface.updateDB(
          "UPDATE appregistry SET use_calendar = '"
              + usesCalendar
              + "' "
              + " , use_dialog = '"
              + usesDialog
  ...
  public String addAdminDialog(String portfolio) {
    ...
    sbuf.append(
        "<P><blockquote><font color=\"green\">With this form you can "
    ...
    try {
      ResultSet rs =
          dbInterface.callDB("SELECT * from admins WHERE portfolio = '" + portfolio + "' ");
      while (rs.next()) {
  ...
  public String postNewAdmin(HttpServletRequest req, portfolioUser thisUser) {
    ...
    try {
      jlib.updateDB(
          "DELETE from admins WHERE admin = '"
              + newAdmin
              + "' and portfolio = '"
              + portfolio
              + "' ");
      jlib.updateDB(
          "INSERT into admins(portfolio, admin) VALUES ('" + portfolio + "', '" + newAdmin + "') ");
    } catch (Exception ex) {
  ...
  public String postMOTD(HttpServletRequest req, portfolioUser thisUser) {
    ...
    try {
      ...
      jlib.updateDB(
          "INSERT into motd(portfolio, body) "
              + " values('"
              + portfolio
              + "', '"
      ...
      ResultSet students =
          dbInterface.callDB(
              "SELECT username from students WHERE portfolio = " + " '" + portfolio + "' ");
      while (students.next()) {
  ...
  public String enterCalendarElement(HttpServletRequest req, String portfolio) {
    ...
    if (thisURL == null)
      sqlString =
          "INSERT into calendar(valid, description, portfolio) VALUES "
              + " ('"
              + validDate
              + "', '"
              + description
  ...
  public String listStudents(String portfolio) {
    ...
    try {
      rs =
          dbInterface.callDB(
              "SELECT u.fname, u.lname from students s, users u"
                  + " WHERE s.portfolio = '"
                  + portfolio
  ...
  public String editMOTD() {
    ...
    sbuf.append(
        "<blockquote><font color=\"green\">The message of the day "
  ...
  public String editGroupIDs(String portfolio) {
    ...
    try {
      ...
      while (students.next()) {
        ...
        sbuf.append(
            "<P>" + realname + "<input type='text' name='" + userID + "' size='6' MAXLENGTH='4'>");
      }
    } catch (Exception ex) {
  ...
  public String assignRolesForm(portfolioUser thisUser) {
    ...
    ResultSet rs = dbInterface.callDB("SELECT * from roles " + " ORDER by id ASC");
    sbuf.append("<ul>\n");
    try {
      while (rs.next()) {
        sbuf.append("<li><b>" + rs.getString("id") + ":</b> " + rs.getString("name") + "</li>\n");
      }
    } catch (Exception ex) {
    ...
    sbuf.append(
        "<table border=\"1\"><tr><th>UserName</th><th>Old Role</th>" + " <th>New Role</th></tr>\n");
    try {
      while (rs.next()) {
        sbuf.append(
            "<tr><td>"
  ...
  public String classPortfolio(String portfolio) {
    ...
    try {
      ...
      if (rs.next()) motd = rs.getString("body");
    } catch (Exception ex) {
      System.err.println(ex);
    }
