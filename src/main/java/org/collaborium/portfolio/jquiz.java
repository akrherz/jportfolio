...
/**
 * Portfolio Application Suite Quiz Engine Servlet that does the dirty work for the quiz engine
 *
 * @author Doug Fils
...
public class jquiz extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     *
     * <p>h | quiz help Box p | view Quiz r | review Quizes t | take Quiz default page
     */
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
     ...
     * Switching logic
     *
     * <p>h | post a dialog response u | input the Quiz allready default | ask why are you here
     */
    switch (callMethod.charAt(0)) {
      case 'u':
  ...
  public static String introduction() {
    ...
    sbuf.append(
        "<font class=\"Instructions\"><blockquote>This page contains "
            + "quiz links and \n"
  ...
  public static String quizBoxHelp() {
    ...
    sbuf.append(
        "<font color=\"green\"><blockquote>The box on the left hand "
  ...
  public static String reviewQuizzes(portfolioUser thisUser) {
    ...
    sbuf.append(
        "<font color=\"green\"><blockquote>Note that quizzes "
    ...
    sbuf.append(
        "<TABLE bgcolor=\"white\" width=\"100%\" cellpadding=\"2\" " + " cellspacing=\"0\">\n");

    sbuf.append("<TR><TH colspan=\"3\"><h3>All Taken Quizzes</H3></TH></TR>\n");
    sbuf.append("<TR><TH align=\"left\">Quiz Name:</TH>" + "<TH align=\"left\">Score:</TH></TR>\n");
   ...
   */
  public static String inputQuiz(
      portfolioUser thisUser, HttpServletRequest request, gradebook studentGrades) {
    ...
    try {
      ...
      try {
        /**
         * Lets figure out if this user has taken the quiz or not. If they have, then the attempt
         * variable is reset to this current attempt
         */
        ResultSet rs6 =
            jlib.callDB(
        ...
        ResultSet rs7 =
            jlib.callDB("SELECT attempts FROM quizes " + " WHERE quiznum = " + quizID + " ");
        rs7.next();
        ...
        if (thisQuiz.goodAttempt(thisAttempt)) { // Allowed
          ...
          jlib.updateDB(
              "DELETE from quizattempts WHERE"
                  + " userid = '"
                  + thisUser.getUserID()
                  + "' "
                  + " and portfolio = '"
          ...
          /**
           * Now if the quiz is allowed to be taken more than once, we need to make sure that the
           * student got all the Qs right, before we show the score.
           */
          if (!allowedAttempts.equalsIgnoreCase("1")) {
            ...
            if (numQs.equalsIgnoreCase(myRight)) {
              ...
              jlib.updateDB(
                  "UPDATE quizattempts SET attempt = -1 "
                      + " WHERE userid = '"
                      + thisUser.getUserID()
                      + "' "
                      + " and portfolio = '"
                      ...
                      + "' ");
              sbuf.append(
                  "<BR>Since you answered the questions correctly, "
            ...
            sbuf.append(
                "Your quiz has been saved.  You can find out your "
          ...
          sbuf.append(
              "<font class=\"warn\">Access Warning:</font> This quiz "
      ...
      if (body.length() > 4) {
        ...
        /**
         * ResultSet portHome = dbInterface.callDB("SELECT porthome from portfolios " +" WHERE
         * portfolio = '"+ thisUser.getPortfolio() +"' and " +" porthome != '/jportfolio/servlet/'
         * "); if ( portHome.next() ) { notifyBaseURL = portHome.getString("porthome")
         * +"/quiz/index.jsp"; notifyBaseURL2 = portHome.getString("porthome") +"/dialog/index.jsp";
         * }
         */
        ResultSet maxth =
            jlib.callDB(
  ...
  public static String inputQuizResponse(portfolioUser thisUser, HttpServletRequest request) {
    StringBuffer sbuf = new StringBuffer();
   ...
   */
  public static String takeQuiz(portfolioUser thisUser, String quizID, String thisPageURL) {
  ...
  public static String sideBar(portfolioUser thisUser, gradebook studentGrades) {
    ...
    theBuffer.append("<P>\n");
    if (thisUser.isAdmin()) theBuffer.append(jlib.adminCommands());
  ...
  public static String listQuizzes(portfolioUser thisUser) {
    ...
    try {
      rs =
          jlib.callDB(
              "SELECT quizes.qname, quizes.quiznum, "
      ...
      while (rs.next()) {
        myBuffer.append(
            "<P><a href=\""
                + thisPageURL
                + "?mode=p&qid="
                + rs.getString("quiznum")
                + "\">"
                + rs.getString("qname")
                + "</a><BR>\n"
                + " <blockquote>\n"
                + printOption("1", rs.getString("q1"), " ", "")
                + "<BR>\n"
                + printOption("2", rs.getString("q2"), " ", "")
                + "<BR>\n"
                + printOption("3", rs.getString("q3"), " ", "")
                + "<BR>\n"
                + " </blockquote>\n");
      }
   ...
   */
  public static String viewQuiz(portfolioUser thisUser, gradebook myGrades, String qid) {
    StringBuffer sbuf = new StringBuffer();
    ...
    try {
      if (thisQuiz.isValid() && thisQuiz.hasExpired()) {
        String quizScore = myGrades.getScoreElement("jquiz", qid, thisQuiz.getQuizName());
  ...
  public static String scoreBox(gradebook studentGrades) {
    ...
    myBuffer.append(
        "<TABLE BORDER='0'>\n"
            ...
            + studentGrades.totalScores()
            + "</TD></TR>\n"
            + "<TR><TD>Score on Quizzes:</TD> <TD>"
            + studentGrades.appScores("jquiz")
            + "</TD></TR>\n"
            + "</TABLE>\n");
   ...
   */
  public static String takableQuizzes(
      portfolioUser thisUser, gradebook studentGrades, boolean isShort, String thisPageURL) {
    ...
    if (rs != null) {
      try {
        ...
        while (rs.next()) {
          ...
          scoreName = qName;
          if (isShort && qName.length() > 11) qName = qName.substring(0, 11) + "...";

          // Figure out how many times this student has attempted the quiz
          rs2 =
              jlib.callDB(
                  "SELECT attempt from quizattempts WHERE "
                      + " userid = '"
                      + thisUser.getUserID()
                      + "' "
                      + " and portfolio = '"
          ...
          if (rs2.next()) {
            ...
            if (Integer.parseInt(myAttempts) == -1) // Student got them all right!
            sbuf.append("<TR><TH>" + qName + "</TH><TD>Done</TD>\n");
            else if (Integer.parseInt(allowedAttempts) == 0
                || Integer.parseInt(myAttempts) < Integer.parseInt(allowedAttempts))
              sbuf.append(
                  "<TR><TH><a href='"
                      + thisPageURL
                      + "?mode=t&qid="
                      ...
                      + allowedAttempts
                      + "</TD>\n");
            else sbuf.append("<TR><TH>" + qName + "</TH><TD>Done</TD>\n");
          } else {
            sbuf.append(
                "<TR><TH><a href='"
                    + thisPageURL
          ...
          rs2.close();
          sbuf.append(
              "<TH>" + studentGrades.getScoreElement("jquiz", qID, scoreName) + "</TH></TR>\n");
        } // End of while()
  ...
  public static String intro() {
    ...
    myBuffer.append(
        "This is the Quiz servlet for the Portfolio pacakge.  It "
