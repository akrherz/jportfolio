...
public class jquizAdmin extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    /**
     * What our swith does l | quiz results for a specific quiz q | Post a new Question u | Post a
     * new Quiz
     */
  ...
  public String whichQuiz(portfolioUser thisUser) {
    ...
    sbuf.append(
        "<blockquote><font color=\"green\">Please choose the quiz "
    ...
    try {
      ResultSet rs =
          jlib.callDB(
              "SELECT * from quizes WHERE " + " portfolio = '" + thisUser.getPortfolio() + "' ");
      while (rs.next()) {
  ...
  public String quizResults(portfolioUser thisUser, String quizName) {
    ...
    sbuf.append(
        "<blockquote><font color=\"green\">Here are listed the quiz "
  ...
  public String listQuestions(portfolioUser thisUser) {
    ...
    sbuf.append(
        "<P><blockquote><font color=\"green\">With this dialog you "
  ...
  public String editQuestionDialog(portfolioUser thisUser, HttpServletRequest req) {
    ...
    try {
      ...
      plogger.report("Errr");
      sbuf.append(
          "<b>Error.</b> Could not access questionID: '"
              + qid
              + " "
   ...
   */
  public String inputQuestion(portfolioUser thisUser, HttpServletRequest req) throws myException {
    StringBuffer sbuf = new StringBuffer();
   ...
   */
  public String inputQuiz(portfolioUser thisUser, HttpServletRequest req) throws myException {
    StringBuffer sbuf = new StringBuffer();
    ...
    try {
      ...
      if (!tester.next()) {
        ...
        ResultSet test2 =
            jlib.callDB(
                "SELECT quiznum from quizes WHERE "
                    + " qname = '"
                    + qName
                    + "' and portfolio = '"
                    + thisUser.getPortfolio()
  ...
  public String mkIntroduction() {
    ...
    sbuf.append(
        "<P>The purpose of jquizAdmin is to allow instructor the "
  ...
  public String newQuiz(portfolioUser thisUser) {
    ...
    sbuf.append(
        "<blockquote><font color=\"green\">This dialog also you to "
    ...
    sbuf.append("<P><B>Allready Created Quizzes:</B><BR>" + listQuizzes(thisUser));
  ...
  public String listQuizzes(portfolioUser thisUser) {
    ...
    try {
      rs =
          jlib.callDB(
              "SELECT qname, startdate, stopdate from quizes "
                  + " WHERE portfolio = '"
                  + thisUser.getPortfolio()
                  + "' ");
      while (rs.next()) {
  ...
  public String listQuestions(portfolioUser thisUser, String id) {
    ...
    try {
      rs =
          jlib.callDB(
              "SELECT qid, question from questions "
                  + " WHERE portfolio IN "
      ...
      while (rs.next()) {
        try {
          ...
          sbuf.append(
              "<option value='" + rs.getString("qid") + "'>" + rs.getString("question") + "\n");
        }
      }
