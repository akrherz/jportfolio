...
/**
 * jdot3.java is the dialog engine for the Portfolio Application Suite It is a frontend to the
 * jdot.java, which contains the methods The dialog engine is in it's purest form here. The 3 in
 * jdot is a version number :)
 *
...
public class jdot3 extends HttpServlet {
  ...
  /**
   * Since the jdot methods can be called from JSP, we need to tell the internal methods where to
   * create links to. Maybe this can be handled differently with an external class??
   */
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    /** If the user posted a value for dialogSecurity, then we had better switch the setting */
     ...
     *
     * <p>o | Read more specifying a single message e | Tree Messages view of a threadID i | Print
     * out Servlet Information p | Post a reply to a certain message n | new thread r | readMore of
     * a specific thread z | do Nothing!!
     */
    ...
    try {
      switch (callMethod.charAt(0)) {
        case 'o':
          pageContent.append(
              jdot.readMore(thisUser, STRidnum, "0", thisUser.getDialogSecurity(), thisPageURL));
          break;
          //	case 'b':
        ...
        case 'p':
          pageContent.append(
              jdot.postMessage(threadID, STRidnum, thisPageURL, thisUser.getDialogSecurity()));
          break;
        case 'n':
        ...
        case 'r':
          pageContent.append(
              jdot.readMore(
                  thisUser, STRidnum, skipNum, thisUser.getDialogSecurity(), thisPageURL));
          break;
        case 'z':
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res)
    ...
    myMessage.setClassification(req.getParameter("type"), req.getParameter("mytype"));
    ...
    if (myMessage.getSubject() == null) myMessage.setSubject("No Subject");
    ...
    switch (callMethod.charAt(0)) {
      case 's':
        if (searchStr != null && !searchStr.equals("")) {
          ...
          try {
            pageContent.append(
                jdot.execSearch(thisUser, searchStr, searchCol, skipNum, thisPageURL));
          } catch (Exception ex) {
            plogger.report("Error Searching dialog " + ex);
            pageContent.append("Problem with your query.  Try again.");
          }
        } else pageContent.append("You need to specific something to search on.<BR>\n");
        break;
      case 'z':
  ...
  /**
   * Method that prints out a listing of side bar items, should be modified to fit a certain
   * application
   */
  public String sideBarItems(portfolioUser thisUser, String thisPageURL) {
    StringBuffer sbuf = new StringBuffer();

    sbuf.append(jdot.switchDialogType(thisUser.getDialogSecurity(), thisPageURL));
    sbuf.append("<P>\n");
    sbuf.append(jlib.libPortApps());
    sbuf.append("<P>\n");
    sbuf.append(jlib.currentUsers(thisUser.getPortfolio(), thisUser.getUserID()));
    sbuf.append("<P>\n");
