...
public class jdot {
   ...
   */
  public static String bodyItems(portfolioUser thisUser, String skipNum, String thisPageURL)
    ...
    if (thisUser.getDialogSecurity().equalsIgnoreCase("group")) {
      if (thisUser.isAdmin())
        mySQL =
            "select count(name) as result from dialog where idnum > 10000 "
      ...
      if (numMessages2 > messageBlock) {
        ...
        for (int i = 0; i < ((numMessages2 - 1) / messageBlock + 1); i++) {
          if (!skipNum.equalsIgnoreCase(String.valueOf(i))) {
            sbuf.append(
                "<a href='" + thisPageURL + "?skipNum=" + i + "'>" + (i + 1) + "</a> &nbsp;\n");
          } else {
            sbuf.append("<b>" + (i + 1) + "</b> &nbsp;\n");
          }
        }
    ...
    if (thisUser.getDialogSecurity().equalsIgnoreCase("group"))
      if (thisUser.isAdmin())
        mySQL2 =
            "select *, getRoleName(username, portfolio) as role from "
    ...
    if (qrs != null) sbuf.append(displayMessages(thisUser, qrs, thisPageURL));
   ...
   * @return HTML formatted string
   */
  public static String displayMessages(portfolioUser thisUser, ResultSet rs, String thisPageURL)
      throws SQLException {
    ...
    while (rs.next()) {
      ...
      sbuf.append(myMessage.printStandard() + myMessage.printExtended(thisPageURL));
  ...
  public static String actionBox(portfolioUser thisUser, String thisPageURL) {
    ...
    myBuffer.append(
        "<ul><LI><a href='" + thisPageURL + "?mode=d'>List Discussion Topics.</a></LI>\n");
   ...
   */
  public static String listMessages(ResultSet rs, String thisPageURL) throws SQLException {
    StringBuffer sbuf = new StringBuffer();
  ...
  public static String printOption(String indexVal, String textVal, String lookFor) {
    ...
    if (lookFor == null || lookFor.equalsIgnoreCase(indexVal)) sbuf.append("SELECTED");
    sbuf.append("> " + textVal + "\n");
   ...
   * @return HTML Formatted String
   */
  public static String anonPrint(
      portfolioUser thisUser, ResultSet message, String indentCols, String dataCols) {
    StringBuffer sbuf = new StringBuffer();
    try {
      ...
      sbuf.append(
          "<b>posted on:</b> "
              + stringUtils.gmtDate(message.getTimestamp("date"))
              + "</td>\n"
              + "</tr>\n"
              + "<tr>\n"
              + "  <td class=\"postBottom\" colspan=\""
              + dataCols
              + "\" class=\"postBot\">"
              + "<font class=\""
              + classification
              + "\">"
              + stringUtils.toBR(message.getString("body"))
              + "</font>");

      if (link != null && link.length() > 0)
        sbuf.append(
            "<BR><B>Link Ref:</B> <a target=\"_new\" "
                + " href=\""
                + link
                + "\">"
                + link
   ...
   */
  public static String treeMessages(
      portfolioUser thisUser, String skipNum, String threadID, String thisPageURL)
      throws SQLException {
      ...
      if (thisUser.isAdmin()) {
        SQL =
            "SELECT * from dialog WHERE "
                ...
                + " order by idnum ASC";
        SQL2 =
            "SELECT threadid, subject from dialog WHERE "
                + threadCount
                + " "
                + " and portfolio = '"
                + thisUser.getPortfolio()
    ...
    if (threadID != null) {
      ...
      sbuf.append(
          "<p><a href=\"/jportfolio/jsp/user/threadCat.jsp?threadID="
              + threadID
              + "\">Printable View of Thread</a>\n");
      ...
      while (threads.next()) {
        ...
        threads2 =
            dbInterface.callDB(
                "SELECT * from dialog WHERE idnum > "
                    + startNum
                    + " ::numeric"
                    + " and idnum < ("
                    + startNum
              ...
              while (threads5.next()) {
                ...
                while (threads6.next()) {
                  ...
                  while (threads7.next()) {
                    ...
                    sbuf.append(
                        printMessageLine(thisUser, STRidnum, 6, "1", threads7, thisPageURL));
                  } // while7
                  threads7.close();
                  sbuf.append("</UL>\n");
                } // while6
   ...
   * @return HTML formatted string
   */
  public static String printMessageLine(
    ...
    sbuf.append(
        "<li><div style=\"background: #FFFFFF\">\n"
            + "<a class=\"boxlink\" "
            + " href=\"javascript: setLayerDisplay('post"
            + STRidnum
            + "');\">+/-</a>\n"
            + "<a "
            + "href=\""
  ...
  /**
   * Method to post messages to the database. Thank goodness for the new message container class,
   * since now my job is about 10 times easier
   *
   * @param myMessage message Container
   * @param thisPageURL URl of calling page.
   * @return HTMLformated String
   */
  public static String inputPostFinal(portfolioMessage myMessage, String thisPageURL)
    ...
    if (myMessage.getThreadID().equalsIgnoreCase("new")) {
      ResultSet maxth =
          dbInterface.callDB(
              "select MAX (idnum)+1 as result from dialog"
        ...
        while (students.next()) {
          ...
          jlib.updateDB(
              "INSERT into notify(username, portfolio, program, message) "
                  ...
                  + " '"
                  + notifyBaseURL
                  + "?dialogType=group&mode=r&idnum="
                  + myMessage.getidnum()
                  + "', "
                  + " 'Group Discussion Topic Created: "
                  + myMessage.getAuthorName()
                  + " posted')");
        }
      }
      ...
      ResultSet test2 =
          dbInterface.callDB(
              "SELECT security from dialog " + " WHERE idnum = '" + myMessage.getThreadID() + "' ");
      test2.next();

      newSTRidnum = maxm.getString("result");
      myMessage.setSecurity(test2.getString("security"));
      if (newSTRidnum == null) newSTRidnum = myMessage.getidnum() + "0001"; // first post in the
      // thread
  ...
  public static String postMessage(
      String threadid, String STRidnum, String thisPageURL, String dialogType)
      throws SQLException, ClassNotFoundException {
    StringBuffer theBuffer = new StringBuffer();
  ...
  /**
   * readMore() - given a threadID and skipNum it generates a resultSet to hand off to
   * displayMessage() to view
   */
  public static String readMore(
      portfolioUser thisUser, String idNum, String skipNum, String dialogType, String thisPageURL)
      throws SQLException {
    ...
    ResultSet rs =
        dbInterface.callDB(
            "select * from dialog where idnum > "
                ...
                + " and idnum < "
                + secondOne
                + "0000::numeric ORDER BY date DESC "
                + " LIMIT "
                + messageBlock
                + " OFFSET "
                + skipMessenges);
    ...
    myBuffer.append(
        "<font class=\"bodyText\" size=\"+2\">Responses to " + "Active Post:</font><BR>\n");
    if (rs != null) {
  ...
  /**
   * threadLeader() -method that prints out a special box with the current post that is active
   *
   ...
   * @return String html
   */
  public static String threadLeader(String idNum, int identCols, String thisPageURL)
      throws SQLException {
    StringBuffer sbuf = new StringBuffer();
   ...
   * @return HTML formatted string
   */
  public static String execSearch(
      portfolioUser thisUser,
      String searchStr,
      String searchCol,
      String skipNum,
