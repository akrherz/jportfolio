...
public class gradebook {
  ...
  public String totalScores() {
    ...
    try {
      rs =
          dbInterface.callDB(
              "SELECT sum(score) as ssum from scores "
                  + " WHERE userID = '"
                  + thisUser.getUserID()
                  + "' "
                  + " and portfolio = '"
  ...
  /**
   * This method is for legacy support. Back in the day, I did a stupid thing and stored the
   * quizName in the scores table instead of the qid. The result was bad.
   */
