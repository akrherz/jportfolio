...
/**
 * Servlet that handles administrative duties of the forecast activity in Portfolio.
 *
...
public class forecastAdmin extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (callMethod == null) callMethod = "z";
    ...
    // t == change the time in which forecasts are due...
    {
      case 'f':
        ...
        try {
          ...
          pageContent.append(
              "<P>An error occured processing your forecast"
                  + "<BR> "
                  + ex
      ...
      while (rs.next()) {
        sbuf.append(
            "<TR><TD>"
                ...
                + "</TD>\n"
                + "<TD>"
                + rs.getString("local_high")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("local_low")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("local_prec")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("local_snow")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("float_high")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("float_low")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("float_prec")
                + "</TD>\n"
                + "<TD>"
                + rs.getString("float_snow")
                + "</TD>\n"
                + "</TR>\n");
      }
    } catch (Exception ex) {
  ...
  public String whatTime2Forecast(portfolioUser thisUser, String thisPageURL) {
    ...
    sbuf.append(
        "<blockquote class=\"instructions\">You must specify a local "
            + "time at which the\n"
    ...
    try {
      ResultSet rs =
          dbInterface.callDB(
              "SELECT fxtime from portfolios WHERE "
                  + " portfolio = '"
                  + thisUser.getPortfolio()
                  + "' and fxtime IS NOT NULL");
      if (rs.next())
        sbuf.append(
            "<input type=\"text\" name=\"fxtime\" value "
                + " =\""
                + rs.getString("fxtime")
                + "\" size=\"2\" maxlength=\"2\">\n");
      else sbuf.append("<input type=\"text\" name=\"fxtime\" " + " size=\"2\" maxlength=\"2\">\n");
  ...
  public String makeForecast(
      portfolioUser thisUser, String selectedUserID, String sqlDate, String thisPageURL) {
    StringBuffer sbuf = new StringBuffer();
    ...
    if (sqlDate == null) sqlDate = "'TODAY'::date + '1 day'::interval";
    ...
    sbuf.append(
        "<form method='POST' action='"
            ...
            + " <input type='hidden' name='sqlDate' value=\""
            + sqlDate
            + "\">\n"
            ...
            + "'></TD>\n"
            + "	<TD>"
            + fLib.rainSelect("local_prec", local_prec)
            + "</TD>\n"
            + "	<TD>"
            + fLib.snowSelect("local_snow", local_snow)
            + "</TD>\n"
            + "	</TR>\n"
            + " <TR><TH>Floater:</TH>\n"
            ...
            + "'></TD>\n"
            + "	<TD>"
            + fLib.rainSelect("float_prec", float_prec)
            + "</TD>\n"
            + "	<TD>"
            + fLib.snowSelect("float_snow", float_snow)
            + "</TD>\n"
            + "	</TR></TABLE>\n");
  ...
  public String enterForecast(portfolioUser thisUser, String selectedUserID, HttpServletRequest req)
    ...
    String colNames =
        "(userID, portfolio, day, local_high, local_low, "
  ...
  public String createValidation(portfolioUser thisUser, String sqlDate) {
    ...
    if (thisDay.getValidation()) sbuf.append("<P>Retrieving Previous Validation.\n");
    else sbuf.append("<P>Setting Validation to default values.\n");
    if (thisDay.getClimo()) sbuf.append("<P>Retrieving Climo Information.\n");
    else sbuf.append("<P>Setting Climo to default values.\n");
  ...
  public String enterForecastValidation(portfolioUser thisUser, HttpServletRequest req) {
    ...
    try {
      dbInterface.updateDB(
          "DELETE from forecast_answers WHERE "
              + " portfolio = '"
              + thisUser.getPortfolio()
              + "' and day = '"
              + sqlDate
      ...
      dbInterface.updateDB(
          "DELETE from forecast_climo WHERE  "
              + " portfolio = '"
              + thisUser.getPortfolio()
              + "' and day = '"
              + sqlDate
  ...
  /**
   * Method that creates a new forecasting day. Also puts a message in the calendar about the new
   * forecast that is due
   *
  ...
  public String addForecastingDay(HttpServletRequest request, String portfolio) {
    ...
    try {
      dbInterface.updateDB(
          "DELETE from forecast_days WHERE day = '"
              + dateStr
              + "' "
              + " AND portfolio = '"
              + portfolio
              + "' ");
      dbInterface.updateDB(
          "INSERT into forecast_days VALUES('"
              + portfolio
              + "', "
              + " '"
              + dateStr
  ...
  public String adminOptions(portfolioUser thisUser, String thisPageURL) {
    ...
    sbuf.append(
        "<li><a class=\"commands\" href='" + thisPageURL + "?mode=u'>Enter Validation</a></li>\n");
    sbuf.append(
