 ...
 */
/**
 * I am back and will incorperate the forecast activity into the portfolio system. The transition
 * should be excellent
 *
...
public class forecast extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    // n == select a certain day to view a users forecast for
    {
      case 'c':
        ...
        try {
          pageContent.append(fLib.cumulativeResults(thisUser.getPortfolio(), sort, thisPageURL));
        } catch (Exception ex) {
      ...
      case 'l':
        ...
        try {
          pageContent.append(
              fLib.forecastResults(thisUser.getPortfolio(), sqlDate, sort, thisPageURL));
        } catch (Exception ex) {
  ...
  public String userForecastResults(portfolioUser thisUser, String sqlDate) {
    ...
    sbuf.append(
        "<P>Forecast Numbers for "
            ...
            + "</TD>\n"
            + "	<TD>"
            + thisDay.getLocalHighTemp()
            + "</TD>\n"
            + "	<TD>"
            + thisDay.getLocalLowTemp()
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getLocalPrecCat()
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getLocalSnowCat()
            + "</TD></TR>\n"
            + "  <TR><TD>"
            ...
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getFloaterPrecCat()
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getFloaterSnowCat()
            + "</TD></TR>\n"
            + "</TABLE>\n");
    sbuf.append(
        "<br><b>Confidence:</b> "
            + thisDay.getConfidence()
            + "<br><b>Discussion:</b> "
    ...
    sbuf.append(
        "<P><TABLE>\n"
            ...
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getVLocalPrecCat()
            + " "
            + "	( "
            + thisDay.getVLocalPrecNum()
            + " ) </TD>\n"
            + "	<TD>CAT "
            + thisDay.getVLocalSnowCat()
            + " "
            + "	( "
            ...
            + "</TD>\n"
            + "	<TD>CAT "
            + thisDay.getVFloaterPrecCat()
            + " "
            + "	( "
            + thisDay.getVFloaterPrecNum()
            + " ) </TD>\n"
            + "	<TD>CAT "
            + thisDay.getVFloaterSnowCat()
            + " "
            + "	( "
        ...
        if (thisUser.isAdmin()) {
          sbuf.append(
              "<tr><td></td><td><i>Coinfidence:</i>  "
                  ...
                  + stringUtils.toBR(rs.getString("discussion"))
                  + "</td></tr>\n");
        }
      }
    } catch (Exception ex) {
  ...
  public String mkIntro() {
    ...
    sbuf.append(
        "<P>This is the Forecasting Activity Application inside of "
            + "Portfolio.  Your \n"
            + " portfolio may not be participating in the Activity, so "
            + "this page may be ignored. \n");

    sbuf.append(
        "<P>In the column to the left, you will find options "
  ...
  public String classOptions(portfolioUser thisUser, String thisPageURL) {
    ...
    else sbuf.append("No Forecast for Today!");
    ...
    sbuf.append(
        "<li><a class=\"commands\" href='" + thisPageURL + "?mode=n'>View Forecasts</a></li>\n");
    sbuf.append("</ul>\n");
    ...
    sbuf.append("<P>\n");
    if (thisUser.isAdmin()) sbuf.append(jlib.adminCommands());

    return sbuf.toString();
  } // End of classOptions

  /** */
  public String makeForecast(portfolioUser thisUser, String sqlDate, String thisPageURL) {
    StringBuffer sbuf = new StringBuffer();

    if (sqlDate == null) sqlDate = "tomorrow";

    forecastDay thisDay = new forecastDay(thisUser, sqlDate);
    if (thisDay.getForecast()) sbuf.append("<P>It appears that you are ammending your forecast.\n");
    else sbuf.append("<P>This is your first attempt at this forecast. Good luck.\n");

    sbuf.append(
        "<P>The floater site is "
            + thisDay.getFloaterSite()
            + " "
            + " ( "
    ...
    sbuf.append(
        "<form method='POST' action='"
            ...
            + "'></TD>\n"
            + "	<TD>"
            + fLib.rainSelect("local_prec", thisDay.getLocalPrecCat())
            + "</TD>\n"
            + "	<TD>"
            + fLib.snowSelect("local_snow", thisDay.getLocalSnowCat())
            + "</TD>\n"
            + "	</TR>\n"
            + " <TR><TH>"
            + thisDay.getFloaterSite()
            ...
            + "'></TD>\n"
            + "	<TD>"
            + fLib.rainSelect("float_prec", thisDay.getFloaterPrecCat())
            + "</TD>\n"
            + "	<TD>"
            + fLib.snowSelect("float_snow", thisDay.getFloaterSnowCat())
            + "</TD>\n"
            + "	</TR>"
            + ("<tr><td colspan=5 bgcolor=\"#0000f0\"><font "
            ...
            + thisDay.getDiscussion()
            + "</textarea></td></tr>\n"
            + "</TABLE>\n");

    sbuf.append(
        "<P><input type='submit' value='Make Forecast'>\n"
            + "<input type='reset'>\n"
            + "</form>\n");
  ...
  public boolean isForecastDay(portfolioUser thisUser) {
    ...
    if (!thisUser.doForecast()) return false;
    ...
    try {
      ...
      if (rs.next()) {
        ...
        if (rs2.next()) {
          ...
          java.util.TimeZone tz = java.util.TimeZone.getTimeZone("America/Chicago");
          java.util.Calendar cal = java.util.Calendar.getInstance(tz);
          int currentHour = cal.get(java.util.Calendar.HOUR_OF_DAY);
          // Add debugging for now.
          plogger.report("Current Hour: " + currentHour + " Forecast Time: " + fxtime);
  ...
  public String selectForecastDays(portfolioUser thisUser, String selected) {
    ...
    try {
      rs =
          dbInterface.callDBWithParameters(
              "SELECT * from forecast_days WHERE " + " portfolio = ? ",
              Arrays.asList(thisUser.getPortfolio()));
      while (rs.next()) {
